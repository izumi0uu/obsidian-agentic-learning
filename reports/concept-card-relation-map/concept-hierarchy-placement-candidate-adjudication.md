...
- `agentic learning/wiki/concepts/Approval Gate.md` / `Approval Gate 应放在不可逆`: …t injection 可能诱导 Agent 隐瞒风险，所以审批信息应由系统生成。 ## 边界细节 Approval Gate 应放在不可逆、高影响、跨权限或外部提交动作之前。低风险读取可以自动通过，高风险写入应展示动作、参数、来源和风险。只弹一个“确认吗”但不给可判断信息，不是好的 gate。 它和 [[Gating…
