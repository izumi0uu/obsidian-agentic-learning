...
这个仓库采用 LLM Wiki 思路：资料先进 `raw/`，LLM 再把稳定理解写入 `wiki/`，路线、术语表、问题池和健康检查放在 `maps/`。这样每次 ingest、query、lint 都会沉淀到文件里，而不是消失在聊天记录里。
...
### `agentic learning/daily/`

每日学习模板。当前已经有 30 天起步计划，用来把学习动作拆小：
...
3. 连接：把新卡接入 topic、术语表、学习路线。
4. 追问：把聊天里的好问题写回 `02 问题池.md` 或 `05 Query 写回队列.md`。
5. 复习：每天从 daily 页面做一个最小动作。
...
这个 vault 不是“已经学会 Agent”的证明。它只是把资料、概念、问题和路线放到了一个可维护的系统里。
