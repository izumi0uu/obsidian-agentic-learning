...
freshness: watch
sha256: 5eff908a16bf35a021584a7fe726eff7faa9e755b8967f5b72d95eebc8c3ce6e
related:
...
原始链接：https://xiaolinnote.com/ai/tools/10_mcp_vs_skill.html
...
原创[公众号@小林面试笔记](https://xiaolinnote.com)[[Tool Use|Tools]]大约 10 分钟约 2855 字

---

# [10. MCP 和 Agent Skill 的区别是什么？](#_10-mcp-和-agent-skill-的区别是什么)
...
### [从定位说起：两者解决的不是同一个问题](#从定位说起-两者解决的不是同一个问题)
...
### [MCP：让 Agent 有「手」](#mcp-让-agent-有「手」)

理解了两者的定位差异，先来展开看看 MCP。为什么说它是 Agent 的「手」？因为如果没有 MCP，Agent 就只会「说话」，不会「做事」。

MCP Server 暴露的 Tools、Resources、Prompts 就是 Agent 操作外部世界的手段。模型通过 [[Tool Calling|Function Calling]] 触发这些工具，工具执行完把结果喂回对话。MCP 的粒度是「原子操作」，`read_file(path)` 是一个 Tool，`query_database(sql)` 是一个 Tool，每次调用做一件明确完整的事，执行结果立刻返回。模型能看到每个 Tool 的完整 schema，包括它叫什么名字、接受什么参数、返回什么格式。这种精确的可见性正是模型能准确判断「这个问题该调哪个工具」的基础。

简单说，MCP 让 Agent 从「只会聊天」进化成了「能真正干活」，这是一切上层能力的前提。

### [Skill：给 Agent 一份「操作手册」](#skill-给-agent-一份「操作手册」)
...
- [跳至主要內容](https://xiaolinnote.com/ai/tools/10_mcp_vs_skill.html#main-content)
...
- [10. MCP 和 Agent Skill 的区别是什么？](https://xiaolinnote.com/ai/tools/10_mcp_vs_skill.html#_10-mcp-和-agent-skill-的区别是什么)
- [💡 简要回答](https://xiaolinnote.com/ai/tools/10_mcp_vs_skill.html#💡-简要回答)
- [📝 详细解析](https://xiaolinnote.com/ai/tools/10_mcp_vs_skill.html#📝-详细解析)
- [从定位说起：两者解决的不是同一个问题](https://xiaolinnote.com/ai/tools/10_mcp_vs_skill.html#从定位说起-两者解决的不是同一个问题)
- [MCP：让 Agent 有「手」](https://xiaolinnote.com/ai/tools/10_mcp_vs_skill.html#mcp-让-agent-有「手」)
- [Skill：给 Agent 一份「操作手册」](https://xiaolinnote.com/ai/tools/10_mcp_vs_skill.html#skill-给-agent-一份「操作手册」)
- [SKILL.md](http://SKILL.md)
- [两者怎么配合工作](https://xiaolinnote.com/ai/tools/10_mcp_vs_skill.html#两者怎么配合工作)
- [🎯 面试总结](https://xiaolinnote.com/ai/tools/10_mcp_vs_skill.html#🎯-面试总结)
- [上一页 9. Skill 是什么？](https://xiaolinnote.com/ai/tools/9_skill.html)
- [下一页 11. Function Calling、Skill、MCP 这三个有什么区别？](https://xiaolinnote.com/ai/tools/11_fc_skill_mcp.html)
