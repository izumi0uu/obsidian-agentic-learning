...
MCP Server 暴露的 [[Tool Use|Tools]]、Resources、Prompts 就是 Agent 操作外部世界的手段。模型通过 [[Tool Calling|Function Calling]] 触发这些工具，工具执行完把结果喂回对话。MCP 的粒度是「原子操作」，`read_file(path)` 是一个 Tool，`query_database(sql)` 是一个 Tool，每次调用做一件明确完整的事，执行结果立刻返回。模型能看到每个 Tool 的完整 schema，包括它叫什么名字、接受什么参数、返回什么格式。这种精确的可见性正是模型能准确判断「这个问题该调哪个工具」的基础。
