...
freshness: watch
sha256: bd5a3c59a3e04fb5a1e3d612b7b0111dcf1b050ba59007d5e27275e2b2dcbb1c
related:
...
传统 AI 是你问一个问题它回答一个问题，每次都是独立的，被动响应；而 Agent 有自己的[[Planning|规划能力]]，你给它一个复杂目标，它会自己把任务拆成多步，通过调工具、访问[[Memory|记忆]]、感知环境来一步步执行，直到完成。
...
**第一件：工具调用（[[Tool Calling|Tool]] Use）**，这是让 Agent 从「说话」变成「做事」的关键。Agent 能调用外部工具，比如搜索引擎、代码执行器、数据库、API 等等。不过这里有一个容易误解的地方：不是模型自己执行，而是模型「告诉你该调什么」，你的代码去真正执行，结果再反馈给模型。模型始终只是大脑，不是手脚。
