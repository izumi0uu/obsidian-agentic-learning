...
freshness: watch
sha256: 2d3bfaee447f5ee6a0f430c591ce01d7947d2065d1069cb305c67a7b65926f43
related:
...
原始链接：https://xiaolinnote.com/ai/tools/2_llm_tool_learning.html
...
[公众号@小林面试笔记](https://xiaolinnote.com)大约 11 分钟约 3421 字
...
👔面试官：SFT 只解决了「会不会调」的问题，但「该不该调」呢？用户问「1+1 等于几」，模型也去调计算器工具，这不是画蛇添足吗？SFT 之后还有一个关键阶段你完全没提到，回去把 [[LLM Training Pipeline|RLHF]] 在工具调用训练中的作用搞清楚再来。
...
有一点非常关键：模型全程只是在「下指令」，真正执行工具的是你的代码，不是模型本身。这套「模型决策、代码执行」的运行时机制，就是我们常说的 **[[Tool Calling|Function Calling]]**。

## [📝 详细解析](#📝-详细解析)

### [原始 LLM 的世界，为什么不会调工具](#原始-llm-的世界-为什么不会调工具)
...
### [第一阶段：SFT，让模型「见过」工具调用](#第一阶段-sft-让模型「见过」工具调用)
...
### [SFT 的短板，会了，但不知道「该不该调」](#sft-的短板-会了-但不知道「该不该调」)
...
### [第二阶段：RLHF，用反馈建立边界感](#第二阶段-rlhf-用反馈建立边界感)
...
### [一个关键认知：模型只负责「决策」，不负责「执行」](#一个关键认知-模型只负责「决策」-不负责「执行」)
...
- [跳至主要內容](https://xiaolinnote.com/ai/tools/2_llm_tool_learning.html#main-content)
...
- [2. LLM 是如何学会调用外部工具的？](https://xiaolinnote.com/ai/tools/2_llm_tool_learning.html#_2-llm-是如何学会调用外部工具的)
- [💡 简要回答](https://xiaolinnote.com/ai/tools/2_llm_tool_learning.html#💡-简要回答)
- [📝 详细解析](https://xiaolinnote.com/ai/tools/2_llm_tool_learning.html#📝-详细解析)
- [原始 LLM 的世界，为什么不会调工具](https://xiaolinnote.com/ai/tools/2_llm_tool_learning.html#原始-llm-的世界-为什么不会调工具)
- [第一阶段：SFT，让模型「见过」工具调用](https://xiaolinnote.com/ai/tools/2_llm_tool_learning.html#第一阶段-sft-让模型「见过」工具调用)
- [SFT 的短板，会了，但不知道「该不该调」](https://xiaolinnote.com/ai/tools/2_llm_tool_learning.html#sft-的短板-会了-但不知道「该不该调」)
- [第二阶段：RLHF，用反馈建立边界感](https://xiaolinnote.com/ai/tools/2_llm_tool_learning.html#第二阶段-rlhf-用反馈建立边界感)
- [运行时，训练好之后怎么用](https://xiaolinnote.com/ai/tools/2_llm_tool_learning.html#运行时-训练好之后怎么用)
- [一个关键认知：模型只负责「决策」，不负责「执行」](https://xiaolinnote.com/ai/tools/2_llm_tool_learning.html#一个关键认知-模型只负责「决策」-不负责「执行」)
- [🎯 面试总结](https://xiaolinnote.com/ai/tools/2_llm_tool_learning.html#🎯-面试总结)
- [上一页 1. 什么是 Function Calling ？原理是什么？](https://xiaolinnote.com/ai/tools/1_function_calling.html)
- [下一页 3. 大模型的 Function Call 能力是怎么训练出来的？](https://xiaolinnote.com/ai/tools/3_fc_training.html)
