...
这次给大家整理了一套 **AI Agent 和 RAG** 方向的面试题，题目都是我从网上真实面经里收集来的，所以这里的每一道都是货真价实的面试真题，不是我编的，也不是八股文，是真实面试里会被问到的问题。
...
做 AI 应用光让模型能聊天是不够的，你得让它能「干活」，能查数据库、能调 API、能操作文件，这些背后就是工具调用。Function Calling、MCP、Agent Skill 这些概念现在面试问得越来越多，但很多人对它们的理解还停留在「都是调工具的」这个层面，一追问就分不清谁是谁了。涵盖 Function Calling 原理与训练、MCP 协议架构、FC 与 MCP 对比选型、Agent Skill 知识封装、A2A 协议、通信协议对比、LLM 网关等面试题。
