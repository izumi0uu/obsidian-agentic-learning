...
答：
它的价值是发现常规测试覆盖不到的高风险边界情况，尤其对 Agent 很关键，因为 Agent 具备执行能力。红队测试是用攻击者视角主动找Agent系统弱点, 以评估和提升其安全性和鲁棒性。常见手法有越权请求、提示注入、越狱诱导、工具滥用。实践上红队结果应回流到规则[[Guardrails|护栏]]、权限策略和训练数据，形成持续加固闭环。
