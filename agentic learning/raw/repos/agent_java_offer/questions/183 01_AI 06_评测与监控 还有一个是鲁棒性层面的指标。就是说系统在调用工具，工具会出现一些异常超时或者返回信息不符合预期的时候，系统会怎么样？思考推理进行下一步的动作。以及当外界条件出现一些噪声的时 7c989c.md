...
freshness: watch
sha256: 95a6ab7f5948cb6205394d4d634400b91ab2e13ae49bc64286f1c7660ef9c153
...
父级题组：评估指标:如何评估一个AlAgent的好坏?除了准确率,还会关注哪些指标(如推理速度、成本、幻觉率)
...
追问题目：还有一个是鲁棒性层面的指标。就是说系统在调用工具，工具会出现一些异常超时或者返回信息不符合预期的时候，系统会怎么样？思考推理进行下一步的动作。以及当外界条件出现一些噪声的时候，这个系统的[[Task Success Rate|成功率]]是否会保持在一个比较稳定的水平？

出现位置：[[Evaluation|评估指标]]:如何评估一个AlAgent的好坏?除了准确率,还会关注哪些指标(如推理速度、成本、幻觉率)
