---
type: source
source_type: repo
site: github.com
repo: guoguo-tju/agent_java_offer
topic:
  - interview
  - ai
  - agent
  - rag
status: inbox
created: 2026-05-09
updated: 2026-05-15
url: https://github.com/guoguo-tju/agent_java_offer/blob/12bf4c915cca01f513e040935e1917d3687f8b35/docs/interview_prep/01_AI/03_RAG/01_%E6%A0%B8%E5%BF%83%E9%97%AE%E7%AD%94.md
source: https://github.com/guoguo-tju/agent_java_offer
source_path: docs/interview_prep/01_AI/03_RAG/01_核心问答.md
commit: 12bf4c915cca01f513e040935e1917d3687f8b35
entry_type: question
direction: 01_AI
category: 03_RAG
last_checked: 2026-05-09
freshness: watch
sha256: fad205379e496ef28e899ec207ffe4a20efaf056353573bc1e3d4434dc5aef06
license: CC BY-NC 4.0
related:
  - "[[agent_java_offer Repo]]"
  - "[[raw/repos/agent_java_offer/agent_java_offer 面试题索引]]"
  - "[[资料收集索引]]"
  - "[[Evaluation]]"
  - "[[Benchmark]]"
  - "[[Task Success Rate]]"
  - "[[Reranking]]"
  - "[[Retriever]]"
  - "[[RAG]]"
  - "[[Guardrails]]"
  - "[[Query Rewrite]]"
  - "[[RAG 主题]]"
---

# [[Query Rewrite|问题改写]]如何设计的？单轮改写还是多轮改写？

原始仓库：<https://github.com/guoguo-tju/agent_java_offer>  
原始文件：[docs/interview_prep/01_AI/03_RAG/01_核心问答.md](https://github.com/guoguo-tju/agent_java_offer/blob/12bf4c915cca01f513e040935e1917d3687f8b35/docs/interview_prep/01_AI/03_RAG/01_%E6%A0%B8%E5%BF%83%E9%97%AE%E7%AD%94.md)  
提交：`12bf4c9`  
分类：`01_AI` / `03_RAG`  
条目类型：`question`  
许可证：CC BY-NC 4.0；本页保留为 raw source evidence，后续再决定是否拆入概念卡或面试复盘页。

## 相关知识 wiki

- [[Evaluation]]
- [[Benchmark]]
- [[Task Success Rate]]
- [[Reranking]]
- [[Retriever]]
- [[RAG]]
- [[Guardrails]]
- [[Query Rewrite]]
- [[RAG 主题]]

## 题目正文

### 1. 子问题：问题改写如何设计的？单轮改写还是多轮改写？

**口述答案（约300字）**：
问题改写目标是提升检索召回，不是把问题“写得更漂亮”。我一般采用“单轮为主，多轮兜底”。单轮改写会做三件事：补全上下文指代、规范术语、拆分复合问题。对于低置信度或召回差的case，再进入多轮改写：先生成若干等价query，再并行检索，最后融合去重。为了避免改写偏题，我会加约束：不改变用户意图、不引入新事实、保留关键实体词。效果[[Evaluation|评估]]看三项：召回命中率提升、相关性提升、端到端[[Task Success Rate|成功率]]变化。若多轮改写提升有限但成本明显增加，就回退到单轮。核心原则是“改写为检索服务”，而不是让模型自由发挥。
**来源**：公开社区资料

## 8. 补充原问：[[Reranking|Rerank]] 和 TopK 设置

## 边界提醒

这是来自面试资料库的原始题目/答案片段，适合练习口述和追问。涉及概念定义、工程边界或时效性事实时，仍需要回到论文、官方文档、框架源码或 `wiki/concepts/` 校准。
