...
答：
有，典型是迭代检索和自适应检索。迭代检索会在初答后做反思，再发起二次检索补证据；自适应检索是在生成过程中遇到不确定点时动态触发检索。还有多源 [[Agentic RAG]]，会同时调用[[Vector Database|向量库]]、图谱、SQL 或搜索 API。它们共同目标是提升复杂问题准确率，但会增加流程复杂度和时延。
...
- **工作流程：**
  ...
  2. **反思与[[Evaluation|评估]]（Reflection）：** [[LLM]]会对初步生成的答案和检索到的上下文进行“反思”。它会评估：当前的信息是否足够支撑答案？答案是否还有不确定或缺失的部分？
...
- **工作流程：** Agent首先对问题进行分解，判断回答这个问题需要哪些信息。然后，它可能会决定：
  ...
  - 从**[[Knowledge Graph|知识图谱]]**中查询结构化的实体关系。
