---
type: source
source_type: repo
site: github.com
repo: guoguo-tju/agent_java_offer
topic:
  - interview
  - ai
  - agent
status: inbox
created: 2026-05-09
updated: 2026-05-09
url: https://github.com/guoguo-tju/agent_java_offer/blob/12bf4c915cca01f513e040935e1917d3687f8b35/docs/interview_prep/01_AI/09_%E8%BF%BD%E5%8A%A0%E8%A1%A5%E5%85%85/01_%E6%A0%B8%E5%BF%83%E9%97%AE%E7%AD%94.md
source: https://github.com/guoguo-tju/agent_java_offer
source_path: docs/interview_prep/01_AI/09_追加补充/01_核心问答.md
commit: 12bf4c915cca01f513e040935e1917d3687f8b35
entry_type: question
direction: 01_AI
category: 09_追加补充
last_checked: 2026-05-09
freshness: watch
sha256: 847948bb2c083db6ec880f2c5460610c4131cb1e1f6579d2995d8320dad8504e
license: CC BY-NC 4.0
related:
  - "[[agent_java_offer Repo]]"
  - "[[raw/repos/agent_java_offer/agent_java_offer 面试题索引]]"
  - "[[资料收集索引]]"
  - "[[Context Engineering]]"
  - "[[Tool Calling]]"
  - "[[Tool Use]]"
  - "[[Agent]]"
  - "[[Agent 主题]]"
---

# 第四课：上下文管理是 Agent 的生命线，必须主动设计

原始仓库：<https://github.com/guoguo-tju/agent_java_offer>  
原始文件：[docs/interview_prep/01_AI/09_追加补充/01_核心问答.md](https://github.com/guoguo-tju/agent_java_offer/blob/12bf4c915cca01f513e040935e1917d3687f8b35/docs/interview_prep/01_AI/09_%E8%BF%BD%E5%8A%A0%E8%A1%A5%E5%85%85/01_%E6%A0%B8%E5%BF%83%E9%97%AE%E7%AD%94.md)  
提交：`12bf4c9`  
分类：`01_AI` / `09_追加补充`  
条目类型：`question`  
许可证：CC BY-NC 4.0；本页保留为 raw source evidence，后续再决定是否拆入概念卡或面试复盘页。

## 相关知识 wiki

- [[Context Engineering]]
- [[Tool Calling]]
- [[Tool Use]]
- [[Agent]]
- [[Agent 主题]]

## 题目正文

### **第四课：[[Context Engineering|上下文管理]]是 [[Agent]] 的生命线，必须主动设计**

上下文窗口 三层递进压缩：先只清理旧的[[Tool Calling|工具调用]]结果（对话主线保留），再在 token 用量接近 87% 时自动触发压缩，最后实在不行了才让 AI 生成全局摘要替换历史。

这个思路对我们很有参考价值：**上下文管理不是一个开关，而是一套策略**。你需要分清楚哪些内容是"可以丢的"（比如中间过程的工具返回结果），哪些是"必须保留的"（比如用户的核心指令和对话主线），然后按优先级来处理。

---

## 边界提醒

这是来自面试资料库的原始题目/答案片段，适合练习口述和追问。涉及概念定义、工程边界或时效性事实时，仍需要回到论文、官方文档、框架源码或 `wiki/concepts/` 校准。
