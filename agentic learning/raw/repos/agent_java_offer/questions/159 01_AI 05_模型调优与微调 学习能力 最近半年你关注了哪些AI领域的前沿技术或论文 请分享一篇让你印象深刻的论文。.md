---
type: source
source_type: repo
site: github.com
repo: "guoguo-tju/agent_java_offer"
topic:
  - "interview"
  - "ai"
  - "agent"
  - "llm-training"
status: inbox
created: 2026-05-09
updated: 2026-05-09
url: "https://github.com/guoguo-tju/agent_java_offer/blob/12bf4c915cca01f513e040935e1917d3687f8b35/docs/interview_prep/01_AI/05_%E6%A8%A1%E5%9E%8B%E8%B0%83%E4%BC%98%E4%B8%8E%E5%BE%AE%E8%B0%83/01_%E6%A0%B8%E5%BF%83%E9%97%AE%E7%AD%94.md"
source: "https://github.com/guoguo-tju/agent_java_offer"
source_path: "docs/interview_prep/01_AI/05_模型调优与微调/01_核心问答.md"
commit: "12bf4c915cca01f513e040935e1917d3687f8b35"
entry_type: "question"
direction: "01_AI"
category: "05_模型调优与微调"
last_checked: 2026-05-09
freshness: watch
sha256: 202d90e45fc3c0009ee40df2409a5d37ca9a381454b9095b5400f30eb771b256
license: "CC BY-NC 4.0"
related:
  - "[[agent_java_offer Repo]]"
  - "[[raw/repos/agent_java_offer/agent_java_offer 面试题索引]]"
  - "[[资料收集索引]]"
  - "[[GraphRAG]]"
  - "[[Neo4j]]"
  - "[[RAG]]"
  - "[[Reranking]]"
  - "[[Retriever]]"
  - "[[Transformer]]"
  - "[[LLM]]"
  - "[[LLM Training Pipeline]]"
  - "[[LLM 主题]]"
---

# 学习能力:最近半年你关注了哪些AI领域的前沿技术或论文?请分享一篇让你印象深刻的论文。

原始仓库：<https://github.com/guoguo-tju/agent_java_offer>  
原始文件：[docs/interview_prep/01_AI/05_模型调优与微调/01_核心问答.md](https://github.com/guoguo-tju/agent_java_offer/blob/12bf4c915cca01f513e040935e1917d3687f8b35/docs/interview_prep/01_AI/05_%E6%A8%A1%E5%9E%8B%E8%B0%83%E4%BC%98%E4%B8%8E%E5%BE%AE%E8%B0%83/01_%E6%A0%B8%E5%BF%83%E9%97%AE%E7%AD%94.md)  
提交：`12bf4c9`  
分类：`01_AI` / `05_模型调优与微调`  
条目类型：`question`  
许可证：CC BY-NC 4.0；本页保留为 raw source evidence，后续再决定是否拆入概念卡或面试复盘页。

## 相关知识 wiki

- [[GraphRAG]]
- [[Neo4j]]
- [[RAG]]
- [[Reranking]]
- [[Retriever]]
- [[Transformer]]
- [[LLM]]
- [[LLM Training Pipeline]]
- [[LLM 主题]]

## 题目正文

### 37.学习能力:最近半年你关注了哪些AI领域的前沿技术或论文?请分享一篇让你印象深刻的论文。

我印象最深的一篇是 LinearRAG (arXiv:2510.10114)。
讲的就是现在主流的引入图关系库，先做图关系库多步推理，然后再进行[[Retriever|向量检索]]，这种方式是比较重的。对，因为首先你构建一个重型的准确的图数据库和复杂推理，这个就比较复杂。所以这篇论文就是想说。可以在业务里优先做轻量结构化的索引，加两个阶段的检索加重排，先拿到稳定的收益。是类似于广告召回中的两段式检索。先去用用户的问题找到相关的实体，再去由实体去找信息。

## 5. 补充原文：BERT、NLP、TFRecord 与 TensorFlow / [[Transformer]] 关系

## 边界提醒

这是来自面试资料库的原始题目/答案片段，适合练习口述和追问。涉及概念定义、工程边界或时效性事实时，仍需要回到论文、官方文档、框架源码或 `wiki/concepts/` 校准。
