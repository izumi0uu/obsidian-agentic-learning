# Toolformer - extracted text

Source PDF: [[Toolformer.pdf]]
...
basic functionality, such as arithmetic or fac- Out of 1400 participants, 400 (or [Calculator(400 / 1400)
...
city councils, to hold their meetings open to the public. a range of tools, including a calculator, a Q&A
...
= Pittsburgh is c
i
1 = What other name is r
i
1 = Steel City L
i
(c
i
1 → Steel City) x* = Pittsburgh is
also known as Pittsburgh known by? < min(L (c 1 → ε), L (ε)) also known as
i i i
[QA(What …?
x i:n = the Steel City c i 2 = Which country is r i 2 = United States L i (c i 2 → United States) → Steel City)]
Pittsburgh in? > min(L (c 2 → ε), L (ε)) the Steel City.
i i i
Figure 2: Key steps in our approach, illustrated for a question answering tool: Given an input text x, we first
sample a position i and corresponding API call candidates c1, c2, . . . , ck. We then execute these API calls and
i i i
...
each such call.
specific tasks.
We represent each API call as a tuple c = (a , i )
c c
Our approach for achieving these goals is based where a is the name of the API and i is the cor-
c c
...
of how an API can be used, we let a LM annotate e(c) = <API> a c (i c ) </API>
a huge language modeling dataset with potential e(c, r) = <API> a (i ) → r </API>
c c
...
calls: script or using a retrieval system to perform search
over a large corpus. The response for each API call
Input: Joe Biden was born in Scranton, Pennsylvania.
c needs to be a single text sequence r .
i i
...
API call c in the sequence x = x , . . . , x , and let
Scranton?")] Pennsylvania. i 1 n
r be the response from the API. Further, given a
i
Input: Coca-Cola, or Coke, is a carbonated soft drink sequence (w i | i ∈ N) of weights, let
manufactured by the Coca-Cola Company.
n
(cid:88)
Output: Coca-Cola, or [QA("What other name is L (z) = − w · log p (x | z, x )
i j−i M j 1:j−1
...
tokens x , . . . , x if the model is prefixed with z.
i n
Input: x We compare two different instantiations of this loss:
Output:
L+ = L (e(c , r ))
i i i i
Figure 3: An exemplary prompt P (x) used to generate L− = min (L (ε), L (e(c , ε)))
i i i i
API calls for the question answering tool.
where ε denotes an empty sequence. The former is
the weighted loss over all tokens x , . . . , x if the
i n
...
tate an example x = x , . . . , x with API calls.
1 n
...
used are shown in Appendix A.2. Let p (z |
M n+1
its input. Given a filtering threshold τ , we thus
z , . . . , z ) be the probability that M assigns to f
1 n
only keep API calls for which
token z as a continuation for the sequence
n+1
z , . . . , z . We first sample up to k candidate posi-
1 n L− − L+ ≥ τ
tions for doing API calls by computing, for each i i f
i ∈ {1, . . . , n}, the probability
holds, i.e., adding the API call and its result reduces
the loss by at least τ , compared to not doing any
f
p = p (<API> | P (x), x )
i M 1:i−1 API call or obtaining no result from it.
that M assigns to starting an API call at position Model Finetuning After sampling and filtering
i. Given a sampling threshold τ , we keep all po- calls for all APIs, we finally merge the remaining
s
sitions I = {i | p > τ }; if there are more than k API calls and interleave them with the original
i s
such positions, we only keep the top k. inputs. That is, for an input text x = x , . . . , x
1 n
For each position i ∈ I, we then obtain up to m with a corresponding API call and result (c , r ) at
i i
API calls c1, . . . , cm by sampling from M given the position i, we construct the new sequence x∗ =
i i
sequence [P (x), x , . . . , x , <API>] as a prefix
1 i−1 3We provide e(c , r ) as a prefix instead of inserting it at
and </API> as an end-of-sequence token.2 position i because M i i i s not yet finetuned on any examples
containing API calls, so inserting it in the middle of x would
2We discard all examples where M does not generate the interrupt the flow and not align with patterns in the pretraining
</API> token. corpus, thus hurting perplexity.

## Page 4

x , e(c , r ), x ; we proceed analogously for snippets from Wikipedia. Compared to our ques-
1:i−1 i i i:n
...
API τ f = 0.5 τ f = 1.0 τ f = 2.0 finetuning.
...
different values of our filtering threshold τ .
f
...
helpful for the model. The thresholds τ s and τ f are shot setup – i.e., models are instructed to solve
...
ples, we assume that the remaining examples are close enough 5This is achieved by manually setting the probability of
to the original distribution so that M’s language modeling the <API> token to 0.
abilities remain unaffected. This assumption is empirically 6We use the original davinci variant that is not finetuned
...
all models in a zero-shot setup by using a more 7An exception to this is if the model’s prediction contains
...
so it may benefit from translating the question into uate all models on TEMPLAMA (Dhingra et al.,
English. Our evaluation metric is the percentage of 2022) and a new dataset that we call DATESET.
times the model’s generation, capped at 10 words, TEMPLAMA is a dataset built from Wikidata that
...
Model TEMPLAMA DATESET Model WikiText CCNet
...
former outperforms all baselines for both TEM- C when API calls are disabled at inference time.8
PLAMA and DATESET. However, closer inspec-
tion shows that improvements on TEMPLAMA 4.4 Scaling Laws
...
one would expect, finetuning on CCNet leads to 8We do not evaluate the perplexity of Toolformer with
slightly improved performance on a different CC- API calls enabled as computing the probability p M (x t |
x , . . . , x ) of token x given x , . . . , x would require
Net subset, but it slightly deteriorates performance 1 t−1 t 1 t−1
...
corresponding score L− − L+ that is used as a fil- approaches that augment language models with
i i
...
As can be seen, high values of L− − L+ typically (Keskar et al., 2019), HTML tags (Aghajanyan
i i
...
The stained-glass Flodden Window in Middleton Parish Church [. . . ] was constructed by Sir 5.49 (cid:51)
...
499 K (1.74 x 287) rather than 735 K which is <API> Calculator(735 / 499) → 1.47 </API> 1.47 1.59 (cid:51)
...
26, 1971. The success of this single established Myles Goodwyn as the band’s main songwriter, 0.92 (cid:55)
...
Schools in Jersey </API> On this page you can search for Universities, Colleges and Business 0.70 (cid:51)
schools in Jersey
...
Table 10: Examples of API calls for different tools, sorted by the value of L− − L+ that is used as a filtering
i i
...
we use values of τ = 0.05 and τ = 1.0 – i.e.,
s f English text before generating API calls. More
...
we set τ = 0.0, k = 20 and m = 10 for these
s
...
comparably small, we additionally set τ = 0.5.
f the MT tool is likely to be helpful. After generating
...
What {day of the week, day of the month, month, 4,000
...
closed.
How many {days, weeks, months, years} {ago 1,200
...
C.1 LAMA and TEMPLAMA D DATESET
