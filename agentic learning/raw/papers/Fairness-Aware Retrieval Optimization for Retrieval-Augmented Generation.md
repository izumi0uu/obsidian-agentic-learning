...
conflicts: []
status: seed
source: https://arxiv.org/abs/2605.15790
related:
  - "[[RAG]]"
  - "[[RAG Evaluation]]"
  - "[[Context Precision]]"
...
- 论文标题：Fairness-Aware Retrieval Optimization for Retrieval-Augmented Generation
- 作者：Yingqi Zhao, Vasilis Efthymiou, Jyrki Nummenmaa, Kostas Stefanidis
- 提交日期：2026-05-15
- 学科：cs.DB, cs.IR
- URL：<https://arxiv.org/abs/2605.15790>
- PDF：<https://arxiv.org/pdf/2605.15790v1>
- 本地 PDF：`assets/Fairness-Aware Retrieval Optimization for Retrieval-Augmented Generation.pdf`（已本地保存；extracted 由 PDF 自动抽取）
- extracted：`extracted/Fairness-Aware Retrieval Optimization for Retrieval-Augmented Generation.extracted.md`
- 阅读优先级：P2
...
这篇更贴近 retrieval optimization：top-k、reranking 和 retrieval-side bias control 可以成为 RAG 可靠性与治理的一部分。

## 一句话

它从 retrieval optimization 角度研究如何控制 RAG 中的 fairness 和 bias propagation。
...
- 原文短摘：
  > Retrieval-Augmented Generation (RAG) improves reliability of large language models by incorporating external knowledge, but the retrieval process can introduce bias that propagates to generated...
- 中文概括：
...
- 原文短摘：
  > This issue is particularly challenging in top-k settings, where multiple documents jointly influence generation.
- 中文概括：
...
- 原文短摘：
  > We propose a fairness-aware retrieval framework that models and controls this bias.
- 中文概括：
...
|---|---|---|---|
| RAG 的公平性风险可以发生在 retrieval/reranking 阶段。 | Abstract / motivation | medium | [[RAG]] |
| 优化 top-k 不能只看 relevance，也要考虑代表性和偏见传播。 | Method | medium | [[RAG Evaluation]] |

边界：没有读到 section/page/figure 时，先写 source note 小节，不要伪造页码或段落级证据。

## 方法 / 机制

- 核心方法：从摘要看，论文围绕 `Fairness-Aware Retrieval Optimization for Retrieval-Augmented Generation` 提出一个前沿方向；具体机制待精读 Method / Framework。
...
- PDF：`assets/Fairness-Aware Retrieval Optimization for Retrieval-Augmented Generation.pdf`
- Extracted Markdown：`extracted/Fairness-Aware Retrieval Optimization for Retrieval-Augmented Generation.extracted.md`
...
|---|---|---|---|
| fairness-aware retrieval | 检索治理候选概念，需和 RAG Evaluation 对齐。 | Abstract / source note | P2 |
| [[Context Precision]] | 相关性以外的 context quality 维度。 | Abstract / source note | P2 |

## 我的疑问

- fairness objective 如何和 relevance objective 共同优化？
- 是否需要 domain-specific protected attributes？
- 对企业知识库 RAG 是否有实用评测方法？
...
- 摘录入口：[[Fairness-Aware Retrieval Optimization for Retrieval-Augmented Generation.extracted#Page 1]]
- arXiv abstract：<https://arxiv.org/abs/2605.15790>
