...
pdf: assets/AstraFlow - Dataflow-Oriented Reinforcement Learning for Agentic LLMs.pdf
extracted: extracted/AstraFlow - Dataflow-Oriented Reinforcement Learning for Agentic LLMs.extracted.md
...
conflicts: []
status: seed
source: https://arxiv.org/abs/2605.15565
related:
  - "[[Agent]]"
  - "[[Tool Use]]"
  - "[[Evaluation]]"
...
- 论文标题：AstraFlow: Dataflow-Oriented Reinforcement Learning for Agentic LLMs
...
- 本地 PDF：`assets/AstraFlow - Dataflow-Oriented Reinforcement Learning for Agentic LLMs.pdf`（已本地保存；extracted 由 PDF 自动抽取）
- extracted：`extracted/AstraFlow - Dataflow-Oriented Reinforcement Learning for Agentic LLMs.extracted.md`
- 阅读优先级：P2
...
这篇偏训练系统，价值在理解 agentic LLM 的 RL 数据流、多 policy 协同和异构计算瓶颈，而不是直接作为应用层架构模板。

## 一句话

AstraFlow 用 dataflow-oriented RL 视角组织 agentic LLM 的训练与多策略协同。
...
- 原文短摘：
  > Reinforcement learning (RL) is increasingly used to improve the reasoning, coding, and tool-use capabilities of large language models, but agentic RL remains prohibitively expensive.
- 中文概括：
...
- 原文短摘：
  > Scaling RL to agentic LLMs requires supporting complex workloads, including multi-policy collaborative training, while efficiently using elastic, heterogeneous, and cross-region compute resources.
- 中文概括：
...
- 原文短摘：
  > Existing LLM RL systems support some of these capabilities, but each new extension often requires dedicated system engineering.
- 中文概括：
...
|---|---|---|---|
| Agentic LLM 训练需要处理工具交互、轨迹和 reward 的数据流组织。 | Abstract / system design | medium | [[Trajectory]] |
| 训练系统的吞吐和异构计算会影响 tool-use / reasoning agent 的可扩展性。 | System / evaluation | medium | [[Evaluation]] |

边界：没有读到 section/page/figure 时，先写 source note 小节，不要伪造页码或段落级证据。

## 方法 / 机制

- 核心方法：从摘要看，论文围绕 `AstraFlow: Dataflow-Oriented Reinforcement Learning for Agentic LLMs` 提出一个前沿方向；具体机制待精读 Method / Framework。
...
- PDF：`assets/AstraFlow - Dataflow-Oriented Reinforcement Learning for Agentic LLMs.pdf`
- Extracted Markdown：`extracted/AstraFlow - Dataflow-Oriented Reinforcement Learning for Agentic LLMs.extracted.md`
...
|---|---|---|---|
| agentic RL training system | 训练系统专题候选，应用层暂不建卡。 | Abstract / source note | P2 |
| [[Trajectory]] | 训练数据和评价证据都依赖轨迹。 | Abstract / source note | P2 |

## 我的疑问

- 它训练的是 policy、tool-use 策略，还是完整 agent loop？
- dataflow abstraction 对开发者是否可见？
- 和 GRPO/RLVR/tool-use RL 的边界是什么？
...
- 摘录入口：[[AstraFlow - Dataflow-Oriented Reinforcement Learning for Agentic LLMs.extracted#Page 1]]
- arXiv abstract：<https://arxiv.org/abs/2605.15565>
