...
pdf: assets/Argus - Evidence Assembly for Scalable Deep Research Agents.pdf
extracted: extracted/Argus - Evidence Assembly for Scalable Deep Research Agents.extracted.md
...
- 论文标题：Argus: Evidence Assembly for Scalable Deep Research Agents
- 作者：Zhen Zhang, Liangcai Su, Zhuo Chen, Xiang Lin, Haotian Xu, Simon Shaolei Du, Kaiyu Yang, Bo An, Lidong Bing, Xinyu Wang
- 提交日期：2026-05-15
- 学科：cs.CL, cs.AI, cs.IR
- URL：<https://arxiv.org/abs/2605.16217>
- PDF：<https://arxiv.org/pdf/2605.16217v1>
- 本地 PDF：`assets/Argus - Evidence Assembly for Scalable Deep Research Agents.pdf`（已本地保存；extracted 由 PDF 自动抽取）
- extracted：`extracted/Argus - Evidence Assembly for Scalable Deep Research Agents.extracted.md`
- 阅读优先级：P1
...
这篇把 deep research agent 的核心问题放在 evidence assembly：并行搜索、去重、互补证据合并和可引用答案，比“多搜几次”更接近真实研究系统。

## 一句话

Argus 研究如何把多路搜索得到的 evidence 组织成可扩展、可引用的 deep research 输出。
...
- 原文短摘：
  > Deep research agents have achieved remarkable progress on complex information seeking tasks.
- 中文概括：
...
- 原文短摘：
  > Even long ReAct style rollouts explore only a single trajectory, while recent state of the art systems scale inference time compute via parallel search...
- 中文概括：
...
- 原文短摘：
  > Yet deep research answers are composed of complementary pieces of evidence, which parallel rollouts often duplicate rather than complete, yielding diminishing returns while pushing...
- 中文概括：
...
|---|---|---|---|
| Deep research 的瓶颈在证据组装和去重，而不只是搜索次数。 | Abstract / system design | medium | [[Agentic Retrieval]] |
| Aggregator / searcher 分工可以降低长上下文和重复证据压力。 | Method | medium | [[Context Engineering]] |

边界：没有读到 section/page/figure 时，先写 source note 小节，不要伪造页码或段落级证据。

## 方法 / 机制

- 核心方法：从摘要看，论文围绕 `Argus: Evidence Assembly for Scalable Deep Research Agents` 提出一个前沿方向；具体机制待精读 Method / Framework。
...
- PDF：`assets/Argus - Evidence Assembly for Scalable Deep Research Agents.pdf`
- Extracted Markdown：`extracted/Argus - Evidence Assembly for Scalable Deep Research Agents.extracted.md`
...
|---|---|---|---|
| evidence assembly | 可能值得未来建卡，需精读方法和证据格式。 | Abstract / source note | P1 |
| [[Agentic Retrieval]] | search agent 如何围绕问题主动组织检索。 | Abstract / source note | P1 |
| [[Long-Horizon Context Engineering]] | 多轮搜索如何压缩和保留证据。 | Abstract / source note | P1 |

## 我的疑问

- 它如何判定两条 evidence 是重复、互补还是冲突？
- 引用和 answer synthesis 的 faithful 检查在哪里发生？
- 多 agent 并行搜索的成本是否被纳入评估？
...
- 摘录入口：[[Argus - Evidence Assembly for Scalable Deep Research Agents.extracted#Page 1]]
- arXiv abstract：<https://arxiv.org/abs/2605.16217>
