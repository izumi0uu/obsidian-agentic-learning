...
pdf: assets/Context, Reasoning, and Hierarchy - A Cost-Performance Study of Compound LLM Agent Design in an Adversarial POMDP.pdf
extracted: extracted/Context, Reasoning, and Hierarchy - A Cost-Performance Study of Compound LLM Agent Design in an Adversarial POMDP.extracted.md
...
conflicts: []
status: seed
source: https://arxiv.org/abs/2605.16205
related:
  - "[[Context Engineering]]"
  - "[[Planning]]"
...
- 论文标题：Context, Reasoning, and Hierarchy: A Cost-Performance Study of Compound LLM Agent Design in an Adversarial POMDP
...
- 本地 PDF：`assets/Context, Reasoning, and Hierarchy - A Cost-Performance Study of Compound LLM Agent Design in an Adversarial POMDP.pdf`（已本地保存；extracted 由 PDF 自动抽取）
- extracted：`extracted/Context, Reasoning, and Hierarchy - A Cost-Performance Study of Compound LLM Agent Design in an Adversarial POMDP.extracted.md`
- 阅读优先级：P1
...
这篇直接比较 compound LLM agent 的上下文、reasoning mode、层级结构和成本性能，适合训练真实 Agent 架构取舍判断。

## 一句话

它把 compound agent 设计拆成 context、reasoning 和 hierarchy 三类选择，并在 adversarial POMDP 中比较成本与效果。
...
- 原文短摘：
  > Deploying compound LLM agents in adversarial, partially observable sequential environments requires navigating several design dimensions: (1) what the agent sees, (2) how it reasons...
- 中文概括：
...
- 原文短摘：
  > We present a controlled study of compound LLM agent design in CybORG CAGE-2, a cyber defense environment modeled as a Partially Observable Markov Decision...
- 中文概括：
...
|---|---|---|---|
| 更多上下文、更多推理或更多层级不必然带来更好 cost-performance。 | Abstract / study design | medium | [[Context Engineering]] |
| Compound agent 的结构选择需要和任务不确定性、对抗性和预算一起评估。 | Evaluation | medium | [[Evaluation]] |

边界：没有读到 section/page/figure 时，先写 source note 小节，不要伪造页码或段落级证据。

## 方法 / 机制

- 核心方法：从摘要看，论文围绕 `Context, Reasoning, and Hierarchy: A Cost-Performance Study of Compound LLM Agent Design in an Adversarial POMDP` 提出一个前沿方向；具体机制待精读 Method / Framework。
...
- PDF：`assets/Context, Reasoning, and Hierarchy - A Cost-Performance Study of Compound LLM Agent Design in an Adversarial POMDP.pdf`
- Extracted Markdown：`extracted/Context, Reasoning, and Hierarchy - A Cost-Performance Study of Compound LLM Agent Design in an Adversarial POMDP.extracted.md`
...
|---|---|---|---|
| compound LLM agent | 可能是系统设计类概念，需和 [[Agent Framework]] 边界校准。 | Abstract / source note | P1 |
| [[Context Engineering]] | 上下文多少和结构如何影响表现。 | Abstract / source note | P1 |
| [[Multi-agent Orchestration]] | 层级分解是否值得。 | Abstract / source note | P1 |

## 我的疑问

- POMDP 任务和真实 web/coding agent 的差异有多大？
- hierarchy 的收益是否来自分工，还是来自 context isolation？
- 成本统计是否包含 token、工具调用和失败重试？
...
- 摘录入口：[[Context, Reasoning, and Hierarchy - A Cost-Performance Study of Compound LLM Agent Design in an Adversarial POMDP.extracted#Page 1]]
- arXiv abstract：<https://arxiv.org/abs/2605.16205>
