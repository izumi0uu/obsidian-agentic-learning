...
- 本地 PDF：`assets/CANTANTE - Optimizing Agentic Systems via Contrastive Credit Attribution.pdf`（本轮未保存 PDF，仅用远程 PDF 生成 extracted 文本）
- extracted：`extracted/CANTANTE - Optimizing Agentic Systems via Contrastive Credit Attribution.extracted.md`
...
- 支撑概念：
  ...
  - [[Agent Workflow]]
- 证据边界：
  - 支撑 [[Multi-agent Orchestration]]；不能推出 prompt optimization 足以优化所有系统。
...
- 原文短摘：
  > agent prompts as learnable system parameters
- 中文概括：
  - 第一轮只基于 arXiv 摘要录入；精读时要回到 PDF / HTML 核对 section、figure、table。
  - 这部分主要支撑本页的“为什么值得读”和概念拆分，不替代稳定概念卡。
- 我需要理解的机制：
...
- 支撑概念：
  ...
  - [[Agent Workflow]]
- 证据边界：
  - 支撑 [[Prompt]]；不是所有 agent 参数都只有 prompt。
...
- 关键步骤：见 [[CANTANTE - Optimizing Agentic Systems via Contrastive Credit Attribution#需要我读的内容]]。
