...
url: https://arxiv.org/abs/2605.16207
pdf: assets/Confirming Correct, Missing the Rest - LLM Tutoring Agents Struggle Where Feedback Matters Most.pdf
extracted: extracted/Confirming Correct, Missing the Rest - LLM Tutoring Agents Struggle Where Feedback Matters Most.extracted.md
...
conflicts: []
status: seed
source: https://arxiv.org/abs/2605.16207
related:
  - "[[Evaluation]]"
...
- 论文标题：Confirming Correct, Missing the Rest: LLM Tutoring Agents Struggle Where Feedback Matters Most
- 作者：Tahreem Yasir, Wenbo Li, Sam Gilson, Sutapa Dey Tithi, Xiaoyi Tian, Tiffany Barnes
- 提交日期：2026-05-15
- 学科：cs.AI, cs.CL
- URL：<https://arxiv.org/abs/2605.16207>
- PDF：<https://arxiv.org/pdf/2605.16207v1>
- 本地 PDF：`assets/Confirming Correct, Missing the Rest - LLM Tutoring Agents Struggle Where Feedback Matters Most.pdf`（已本地保存；extracted 由 PDF 自动抽取）
- extracted：`extracted/Confirming Correct, Missing the Rest - LLM Tutoring Agents Struggle Where Feedback Matters Most.extracted.md`
- 阅读优先级：P2
...
这篇对 evaluator / critic agent 有启发：Agent 容易确认正确答案，却在部分正确、suboptimal 或错误反馈上失效。

## 一句话

它研究 tutoring agents 在最需要诊断反馈的场景中是否能指出缺失、错误和部分正确之处。
...
- 原文短摘：
  > Effective tutoring requires distinguishing optimal, valid but suboptimal, and incorrect student solutions, a distinction central to intelligent tutoring systems (ITS) but untested for LLM-based...
- 中文概括：
...
|---|---|---|---|
| 反馈型 Agent 的难点不在确认正确，而在诊断错误和遗漏。 | Abstract / motivation | medium | [[Evaluation]] |
| 教育反馈质量需要比最终正确率更细的评价维度。 | Evaluation | medium | [[Agent Robustness]] |

边界：没有读到 section/page/figure 时，先写 source note 小节，不要伪造页码或段落级证据。

## 方法 / 机制

- 核心方法：从摘要看，论文围绕 `Confirming Correct, Missing the Rest: LLM Tutoring Agents Struggle Where Feedback Matters Most` 提出一个前沿方向；具体机制待精读 Method / Framework。
...
- PDF：`assets/Confirming Correct, Missing the Rest - LLM Tutoring Agents Struggle Where Feedback Matters Most.pdf`
- Extracted Markdown：`extracted/Confirming Correct, Missing the Rest - LLM Tutoring Agents Struggle Where Feedback Matters Most.extracted.md`
...
|---|---|---|---|
| feedback agent evaluation | critic/evaluator agent 评测候选，先停在 source。 | Abstract / source note | P2 |
| [[Agent Robustness]] | 面对部分正确输入时的稳健反馈。 | Abstract / source note | P2 |

## 我的疑问

- 它如何区分 correct、partially correct、suboptimal 和 incorrect？
- 反馈质量由人评、rubric 还是自动 judge 评估？
- 能否迁移到 code review / tutor / reviewer agent？
...
- 摘录入口：[[Confirming Correct, Missing the Rest - LLM Tutoring Agents Struggle Where Feedback Matters Most.extracted#Page 1]]
- arXiv abstract：<https://arxiv.org/abs/2605.16207>
