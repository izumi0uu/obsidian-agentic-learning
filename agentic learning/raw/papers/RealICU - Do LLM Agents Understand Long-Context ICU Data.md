...
- 本地 PDF：`assets/RealICU - Do LLM Agents Understand Long-Context ICU Data.pdf`（已本地保存；extracted 由 PDF 自动抽取）
- extracted：`extracted/RealICU - Do LLM Agents Understand Long-Context ICU Data.extracted.md`
...
- 原文短摘：
  > Evaluating AI models against such labels therefore rewards behavioral imitation rather than clinical correctness.
- 中文概括：
  - 这段原文直接支撑本块阅读目标：理解为什么历史 clinician actions 不一定是最优 ground truth。
...
- 原文短摘：
  > We formulate four physician-motivated tasks: assessPatient Status,Acute Problems,Recommended Actions, andRed Flagactions that risk unsafe outcomes.
- 中文概括：
  - 这段原文直接支撑本块阅读目标：读 Patient Status、Acute Problems、Recommended Actions、Red Flag actions 四类任务。
  - 当前摘录只证明作者在摘要或第一页如何界定问题、机制或结果；后续写稳定概念卡时仍要回正文核对方法、实验设置和限制。
- 我需要理解的机制：
  1. 原文里的对象、动作、约束和评价层级分别是什么。
  2. 这条证据属于问题定义、方法机制、数据/benchmark，还是结果 claim。
...
- 原文短摘：
  > We partition each trajectory with 30-min windows and release two datasets: RealICU-Gold with 930-window annotations from 94 MIMIC-IV patients, and RealICU-Scale with 11,862 windows extended byOracle, a physician-validated LLM hindsight labeler.
- 中文概括：
  - 这段原文直接支撑本块阅读目标：看 ICU trajectory 如何按时间窗构造长上下文评估。
  - 当前摘录只证明作者在摘要或第一页如何界定问题、机制或结果；后续写稳定概念卡时仍要回正文核对方法、实验设置和限制。
- 我需要理解的机制：
  1. 原文里的对象、动作、约束和评价层级分别是什么。
  2. 这条证据属于问题定义、方法机制、数据/benchmark，还是结果 claim。
...
- 核心方法：从本地 extracted Page 1 / Abstract 可确认，论文核心机制与「Four physician-motivated tasks」相关：We formulate four physician-motivated tasks: assessPatient Status,Acute Problems,Recommended Actions, andRed Flagactions that risk unsafe outcomes.
...
- 和相邻方法的差别：当前 Abstract 支持的差别线索是「behavior imitation problem / Four physician-motivated tasks / 30-min windows / long trajectories」；不要把标题术语直接升格为稳定概念。

## 实验 / 证据

- 数据集 / benchmark：从本地 extracted Page 1 / Abstract 可确认的证据线索：We partition each trajectory with 30-min windows and release two datasets: RealICU-Gold with 930-window annotations from 94 MIMIC-IV patients, and RealICU-Scale with 11,862 windows extended byOracle, a physician-validated LLM hindsight labeler.
