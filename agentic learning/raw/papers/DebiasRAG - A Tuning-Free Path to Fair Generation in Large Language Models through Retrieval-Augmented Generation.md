...
pdf: assets/DebiasRAG - A Tuning-Free Path to Fair Generation in Large Language Models through Retrieval-Augmented Generation.pdf
extracted: extracted/DebiasRAG - A Tuning-Free Path to Fair Generation in Large Language Models through Retrieval-Augmented Generation.extracted.md
...
conflicts: []
status: seed
source: https://arxiv.org/abs/2605.16113
related:
  - "[[RAG]]"
  - "[[RAG Evaluation]]"
  - "[[Context Precision]]"
...
- 论文标题：DebiasRAG: A Tuning-Free Path to Fair Generation in Large Language Models through Retrieval-Augmented Generation
...
- 本地 PDF：`assets/DebiasRAG - A Tuning-Free Path to Fair Generation in Large Language Models through Retrieval-Augmented Generation.pdf`（已本地保存；extracted 由 PDF 自动抽取）
- extracted：`extracted/DebiasRAG - A Tuning-Free Path to Fair Generation in Large Language Models through Retrieval-Augmented Generation.extracted.md`
- 阅读优先级：P2
...
这篇把 RAG 从 factuality 扩展到 bias mitigation：retrieval 不只补事实，也会影响生成公平性和偏见传播。

## 一句话

DebiasRAG 研究不微调模型时，如何通过检索增强路径改善生成公平性。
...
- 原文短摘：
  > Large language models (LLMs) have achieved unprecedented success due to their exceptional generative capabilities.
- 中文概括：
...
- 原文短摘：
  > However, because they depend on knowledge encapsulated from training corpora, they may produce hallucinations, stereotypes, and socially biased content.
- 中文概括：
...
- 原文短摘：
  > In particular, LLMs are prone to prejudiced responses involving race, gender, and age, which are collectively referred to as social biases.
- 中文概括：
...
|---|---|---|---|
| RAG 可以影响生成偏见，而不只是事实正确性。 | Abstract / motivation | medium | [[RAG]] |
| Tuning-free 方法需要证明 retrieval 选择确实改善 fairness 而不伤害 factuality。 | Evaluation | medium | [[RAG Evaluation]] |

边界：没有读到 section/page/figure 时，先写 source note 小节，不要伪造页码或段落级证据。

## 方法 / 机制

- 核心方法：从摘要看，论文围绕 `DebiasRAG: A Tuning-Free Path to Fair Generation in Large Language Models through Retrieval-Augmented Generation` 提出一个前沿方向；具体机制待精读 Method / Framework。
...
- PDF：`assets/DebiasRAG - A Tuning-Free Path to Fair Generation in Large Language Models through Retrieval-Augmented Generation.pdf`
- Extracted Markdown：`extracted/DebiasRAG - A Tuning-Free Path to Fair Generation in Large Language Models through Retrieval-Augmented Generation.extracted.md`
...
|---|---|---|---|
| RAG bias mitigation | RAG 治理子方向，先停在 source。 | Abstract / source note | P2 |
| [[RAG Evaluation]] | 评估应覆盖 fairness / bias 之外的传统指标。 | Abstract / source note | P2 |

## 我的疑问

- 它如何定义 fair generation？
- 检索语料本身有偏时如何处理？
- 公平性提升是否牺牲 context relevance 或 answer correctness？
...
- 摘录入口：[[DebiasRAG - A Tuning-Free Path to Fair Generation in Large Language Models through Retrieval-Augmented Generation.extracted#Page 1]]
- arXiv abstract：<https://arxiv.org/abs/2605.16113>
