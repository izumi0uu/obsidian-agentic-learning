...
- 为什么必读：定位论文的核心证据。
- 原文短摘：
  > Key challenges are that current methods represent reasoning through free-form natural language, where intermediate states are implicit, retrieval queries can drift from intended entities, and errors are detected by the same model that produces them making self-reflection an ...
- 中文概括：
...
- 为什么必读：定位论文的核心证据。
- 原文短摘：
  > Motivated by this, we introduce PyRAG, a framework that reformulates multi-hop RAG as program synthesis and execution.
- 中文概括：
  - 这段原文直接支撑本块阅读目标：定位论文的核心证据。
  - 当前摘录只证明作者在摘要或第一页如何界定问题、机制或结果；后续写稳定概念卡时仍要回正文核对方法、实验设置和限制。
- 我需要理解的机制：
...
- 为什么必读：定位论文的核心证据。
- 原文短摘：
  > Instead of free-form reasoning trajectories, PyRAG represents the reasoning process as an executable Python program over retrieval and QA tools, exposing intermediate states as variables, producing deterministic feedback through execution, and yielding an inspectable trace of the entire ...
- 中文概括：
  - 这段原文直接支撑本块阅读目标：定位论文的核心证据。
  - 当前摘录只证明作者在摘要或第一页如何界定问题、机制或结果；后续写稳定概念卡时仍要回正文核对方法、实验设置和限制。
- 我需要理解的机制：
...
- 核心方法：从本地 extracted Page 1 / Abstract 可确认，论文核心机制与「program representation」相关：Motivated by this, we introduce PyRAG, a framework that reformulates multi-hop RAG as program synthesis and execution.
...
- 和相邻方法的差别：当前 Abstract 支持的差别线索是「natural-language reasoning brittleness / program representation / inspectable trace」；不要把标题术语直接升格为稳定概念。

## 实验 / 证据

- 数据集 / benchmark：从本地 extracted Page 1 / Abstract 可确认的证据线索：Instead of free-form reasoning trajectories, PyRAG represents the reasoning process as an executable Python program over retrieval and QA tools, exposing intermediate states as variables, producing deterministic feedback through execution, and yielding an inspectable trace of the entire ...
