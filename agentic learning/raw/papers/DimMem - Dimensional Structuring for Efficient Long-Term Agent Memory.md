...
pdf: assets/DimMem - Dimensional Structuring for Efficient Long-Term Agent Memory.pdf
extracted: extracted/DimMem - Dimensional Structuring for Efficient Long-Term Agent Memory.extracted.md
...
conflicts: []
status: seed
source: https://arxiv.org/abs/2605.15759
related:
  - "[[Long-term Memory]]"
  - "[[Memory]]"
  - "[[Episodic Memory]]"
...
- 论文标题：DimMem: Dimensional Structuring for Efficient Long-Term Agent Memory
- 作者：Wentao Qiu, Haotian Hu, Fanyi Wang, Jinwei Kong, Yu Zhang
- 提交日期：2026-05-15
- 学科：cs.CL
- URL：<https://arxiv.org/abs/2605.15759>
- PDF：<https://arxiv.org/pdf/2605.15759v1>
- 本地 PDF：`assets/DimMem - Dimensional Structuring for Efficient Long-Term Agent Memory.pdf`（已本地保存；extracted 由 PDF 自动抽取）
- extracted：`extracted/DimMem - Dimensional Structuring for Efficient Long-Term Agent Memory.extracted.md`
- 阅读优先级：P1
...
这篇把 agent memory 从粗粒度对话总结推进到有字段的 atomic memory units，适合思考长期记忆的可检索、可更新和可解释结构。

## 一句话

DimMem 用维度化结构组织长期 Agent memory，试图在 fidelity、efficiency 和 structured recall 之间折中。
...
- 原文短摘：
  > However, existing memory systems often face a fidelity--efficiency trade-off: raw dialogue histories are expensive, while flat facts or summaries may discard the structure needed...
- 中文概括：
...
- 原文短摘：
  > We propose \textbf{DimMem}, a lightweight dimensional memory framework that represents each memory as an atomic, typed, and self-contained unit with explicit fields such as...
- 中文概括：
...
|---|---|---|---|
| 长期 memory 不应只是把对话摘要重新塞回上下文。 | Abstract / motivation | medium | [[Long-term Memory]] |
| 字段化 memory units 可以支持更可控的 recall 和更新。 | Method | medium | [[Memory]] |

边界：没有读到 section/page/figure 时，先写 source note 小节，不要伪造页码或段落级证据。

## 方法 / 机制

- 核心方法：从摘要看，论文围绕 `DimMem: Dimensional Structuring for Efficient Long-Term Agent Memory` 提出一个前沿方向；具体机制待精读 Method / Framework。
...
- PDF：`assets/DimMem - Dimensional Structuring for Efficient Long-Term Agent Memory.pdf`
- Extracted Markdown：`extracted/DimMem - Dimensional Structuring for Efficient Long-Term Agent Memory.extracted.md`
...
|---|---|---|---|
| dimensional memory | 可能是 agent memory 的结构化变体，先等精读。 | Abstract / source note | P1 |
| [[Long-term Memory]] | 长期记忆的结构和检索边界。 | Abstract / source note | P1 |
| [[Context Engineering]] | memory 入窗前如何被选择和压缩。 | Abstract / source note | P1 |

## 我的疑问

- 维度字段是人工设计、模型抽取，还是动态学习？
- 记忆更新是否有遗忘、冲突和 provenance 机制？
- 和向量检索、knowledge graph memory 的边界是什么？
...
- 摘录入口：[[DimMem - Dimensional Structuring for Efficient Long-Term Agent Memory.extracted#Page 1]]
- arXiv abstract：<https://arxiv.org/abs/2605.15759>
