...
pdf: assets/BioXArena - Benchmarking LLM Agents on Multi-Modal Biomedical Machine Learning Tasks.pdf
extracted: extracted/BioXArena - Benchmarking LLM Agents on Multi-Modal Biomedical Machine Learning Tasks.extracted.md
...
conflicts: []
status: seed
source: https://arxiv.org/abs/2605.15766
related:
  - "[[Benchmark]]"
  - "[[Evaluation]]"
  - "[[Agent Robustness]]"
...
- 论文标题：BioXArena: Benchmarking LLM Agents on Multi-Modal Biomedical Machine Learning Tasks
- 作者：Loka Li, Duzhen Zhang, Xingbo Du, Leonard Song, Zixiao Wang, Assanali Aukenov, Noel Thomas, Shakhnazar Sailaukan, Yonghan Yang, Feilong Chen, Jiahua Dong, Kun Zhang, Bin Zhang, Le Song
- 提交日期：2026-05-15
- 学科：cs.CE
- URL：<https://arxiv.org/abs/2605.15766>
- PDF：<https://arxiv.org/pdf/2605.15766v1>
- 本地 PDF：`assets/BioXArena - Benchmarking LLM Agents on Multi-Modal Biomedical Machine Learning Tasks.pdf`（已本地保存；extracted 由 PDF 自动抽取）
- extracted：`extracted/BioXArena - Benchmarking LLM Agents on Multi-Modal Biomedical Machine Learning Tasks.extracted.md`
- 阅读优先级：P2
...
这篇是垂直领域 Agent benchmark：不是普通 QA，而是让 Agent 生成 biomedical ML pipeline，适合观察 domain-specific 产出评价。

## 一句话

BioXArena 评估 LLM agents 在多模态 biomedical ML 任务中生成端到端 pipeline 的能力。
...
- 原文短摘：
  > Large language model (LLM) agents are increasingly capable of automating components of machine learning development, yet existing biomedical benchmarks mainly focus on question answering...
- 中文概括：
...
- 原文短摘：
  > We present BioXArena, a biomedical machine learning benchmark designed to evaluate whether agents can generate task-specific model training pipelines for heterogeneous and multi-modal biomedical...
- 中文概括：
...
- 原文短摘：
  > BioXArena contains 76 end-to-end tasks across 9 domains, including sequence modeling, single-cell analysis, structural biology, network biology, chemical biology, perturbation dynamics, phenotype-disease modeling, biomedical...
- 中文概括：
...
|---|---|---|---|
| 垂直领域 Agent benchmark 需要评估实际 pipeline 产出，而不只是问答。 | Abstract / benchmark | medium | [[Benchmark]] |
| Biomedical 多模态任务会放大数据、工具和评估安全边界。 | Evaluation | medium | [[Agent Robustness]] |

边界：没有读到 section/page/figure 时，先写 source note 小节，不要伪造页码或段落级证据。

## 方法 / 机制

- 核心方法：从摘要看，论文围绕 `BioXArena: Benchmarking LLM Agents on Multi-Modal Biomedical Machine Learning Tasks` 提出一个前沿方向；具体机制待精读 Method / Framework。
...
- PDF：`assets/BioXArena - Benchmarking LLM Agents on Multi-Modal Biomedical Machine Learning Tasks.pdf`
- Extracted Markdown：`extracted/BioXArena - Benchmarking LLM Agents on Multi-Modal Biomedical Machine Learning Tasks.extracted.md`
...
|---|---|---|---|
| domain-specific agent benchmark | 垂直 benchmark 候选，先不单独建卡。 | Abstract / source note | P2 |
| [[Trajectory Evaluation]] | pipeline 生成过程比最终答案更重要。 | Abstract / source note | P2 |

## 我的疑问

- benchmark 是否能运行和验证生成 pipeline？
- domain expert label 和自动评分如何结合？
- 多模态 biomedical 风险如何记录？
...
- 摘录入口：[[BioXArena - Benchmarking LLM Agents on Multi-Modal Biomedical Machine Learning Tasks.extracted#Page 1]]
- arXiv abstract：<https://arxiv.org/abs/2605.15766>
