...
- 本地 PDF：`assets/Executable Agentic Memory for GUI Agent.pdf`（已本地保存；extracted 由 PDF 自动抽取）
- extracted：`extracted/Executable Agentic Memory for GUI Agent.extracted.md`
...
- 原文短摘：
  > However, the dominant interaction paradigm remainsmodel-centric and step-wise: at every screen, an LLM must re-interpret the UI, re-decide the next action, and implicitly maintain task progress in its context window.
- 中文概括：
  - 这段原文直接支撑本块阅读目标：理解为什么 GUI Agent 每屏重新解释 UI 会在长程任务里脆弱。
...
- 原文短摘：
  > To ensure efficient planning, we introduce a value-guided graph search where a lightweight Q-function model steers Monte Carlo Tree Search (MCTS) over the KG.
- 中文概括：
  - 这段原文直接支撑本块阅读目标：看 value-guided graph search / MCTS 如何把 memory 用于规划。
  - 当前摘录只证明作者在摘要或第一页如何界定问题、机制或结果；后续写稳定概念卡时仍要回正文核对方法、实验设置和限制。
- 我需要理解的机制：
  1. 原文里的对象、动作、约束和评价层级分别是什么。
  2. 这条证据属于问题定义、方法机制、数据/benchmark，还是结果 claim。
...
- 核心方法：从本地 extracted Page 1 / Abstract 可确认，论文核心机制与「Memory construction pipeline」相关：To this end, we first propose a sample-efficient memory construction pipeline: a state-aware DFS exploration strategy that systematically covers task-relevant transitions with minimal redundant interactions, coupled with state deduplication and action-group mining to compress frequent multi-step routines into ...
...
- 和相邻方法的差别：当前 Abstract 支持的差别线索是「GUI step-wise fragility / Memory construction pipeline / Graph search and Q-function」；不要把标题术语直接升格为稳定概念。

## 实验 / 证据

- 数据集 / benchmark：从本地 extracted Page 1 / Abstract 可确认的证据线索：To ensure efficient planning, we introduce a value-guided graph search where a lightweight Q-function model steers Monte Carlo Tree Search (MCTS) over the KG.
