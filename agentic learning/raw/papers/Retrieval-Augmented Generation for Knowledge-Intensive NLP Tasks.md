...
#### 选读块 1：Section 2.2 / Retriever: DPR

- 位置：Extracted Markdown `Retrieval-Augmented Generation for Knowledge-Intensive NLP Tasks.extracted.md` / Section 2.2 Retriever: DPR
- 为什么选读：这里说明 RAG 原论文的 retriever 使用 DPR，而 DPR 是 bi-encoder architecture，可以支撑 [[Bi-Encoder]] 和 [[Dense Retrieval]] 的历史/架构边界。
- 原文短摘：
  > DPR follows a bi-encoder architecture
- 中文概括：
  - DPR 使用 query encoder 和 document encoder 生成 query/document dense representation。
  - top-k 文档检索被表述为 Maximum Inner Product Search，可用近似搜索加速。
  - 这里支撑的是“RAG 原论文中的 retriever 实现结构”，不等于现代 RAG 必须只用 DPR 或只用 Bi-Encoder。
- 支撑概念：
  - [[Bi-Encoder]]
  - [[Dense Retrieval]]
  - [[Retriever]]
...
- RAG 原论文中的 DPR / bi-encoder retriever 为什么能构建 document index？
...
| RAG 原论文中的 retriever 基于 DPR；DPR 使用 bi-encoder architecture，并把 top-k 检索表述为 MIPS。 | Section 2.2 / Retriever: DPR | high | [[Bi-Encoder]] |
