...
pdf: assets/Library Drift - Diagnosing and Fixing a Silent Failure Mode in Self-Evolving LLM Skill Libraries.pdf
extracted: extracted/Library Drift - Diagnosing and Fixing a Silent Failure Mode in Self-Evolving LLM Skill Libraries.extracted.md
arxiv: https://arxiv.org/abs/2605.19576
doi: 10.48550/arXiv.2605.19576
author:
  - Xing Zhang
  - Yanwei Cui
  - Guanghui Wang
...
conflicts: []
status: seed
source: https://arxiv.org/abs/2605.19576
related:
  - "[[Tool Registry]]"
  - "[[Agent Robustness]]"
  - "[[Evaluation]]"
...
- 论文标题：Library Drift: Diagnosing and Fixing a Silent Failure Mode in Self-Evolving LLM Skill Libraries
- 作者：Xing Zhang, Yanwei Cui, Guanghui Wang, Ziyuan Li, Wei Qiu, Bing Zhu, Peiyang He
- 提交日期：2026-05-19
- 学科：arXiv cs.AI / cs.CL / related（以 arXiv 页面为准）
- URL：<https://arxiv.org/abs/2605.19576>
- PDF：<https://arxiv.org/pdf/2605.19576v1>
- 本地 PDF：`assets/Library Drift - Diagnosing and Fixing a Silent Failure Mode in Self-Evolving LLM Skill Libraries.pdf`（已本地保存；extracted 由 PDF 自动抽取）
- extracted：`extracted/Library Drift - Diagnosing and Fixing a Silent Failure Mode in Self-Evolving LLM Skill Libraries.extracted.md`
- 阅读优先级：P1-A
...
这篇补上长期 skill library 的治理问题：agent 不是只要不断积累技能就会变强，库会漂移、过时、互相冲突，并在 silent failure 前损坏检索和执行。

## 一句话

Library Drift 描述自演化 skill library 因无限积累和缺少 lifecycle management 而发生检索退化、错误注入和性能停滞。
...
- 为什么必读：这是 drift 的最小定义。
- 原文短摘：
  > unbounded skill accumulation without outcome-driven lifecycle management
- 中文概括：
  - 这部分提供第一轮 source-level 证据，帮助判断论文主张属于问题定义、方法机制、评测协议还是应用场景。
  - 技能越多不等于越强，关键是 lifecycle 是否按结果管理。
...
#### 必读块 2：failure symptoms

- 位置：extracted Page 1 / Abstract（必要时连同 Introduction 交叉核对）
- 为什么必读：这三类症状可迁移到 Hermes / OMX skill registry 审计。
- 原文短摘：
  > retrieval degradation, false-positive injections, and performance stagnation
- 中文概括：
  - 这部分提供第一轮 source-level 证据，帮助判断论文主张属于问题定义、方法机制、评测协议还是应用场景。
  - 如何在 end-task 分数下降前从 trace 发现问题。
  - 精读时需要回到正文 section、figure、table 和 limitations，不能只凭摘要外推成稳定工程标准。
- 我需要理解的机制：
  1. 论文对象、输入输出、运行时组件和验证边界分别是什么。
  2. 机制收益来自模型能力、系统结构、数据组织、训练策略还是评测协议。
...
#### 必读块 3：governance recipe

- 位置：extracted Page 1 / Abstract（必要时连同 Introduction 交叉核对）
- 为什么必读：作者提出的是治理配方，不只是诊断名词。
- 原文短摘：
  > outcome-driven retirement + bounded active-cap + meta-skill authoring prior
- 中文概括：
  - 这部分提供第一轮 source-level 证据，帮助判断论文主张属于问题定义、方法机制、评测协议还是应用场景。
  - retirement、active cap、authoring prior 分别解决哪类 drift。
  - 精读时需要回到正文 section、figure、table 和 limitations，不能只凭摘要外推成稳定工程标准。
- 我需要理解的机制：
  1. 论文对象、输入输出、运行时组件和验证边界分别是什么。
  2. 机制收益来自模型能力、系统结构、数据组织、训练策略还是评测协议。
...
- 可能更新的概念卡：[[Tool Registry]]、[[Agent Robustness]]、[[Evaluation]]、[[Trace]]

## 论文主张

| Claim | Evidence anchor | Confidence | Target concept |
|---|---|---|---|
| self-evolving skill library 会出现 library drift，且失败可能先表现为检索和注入问题。 | extracted Page 1 / Abstract | high | [[Tool Registry]] |
| skill library 需要 outcome-driven lifecycle management，而不是无限追加。 | extracted Page 1 / Abstract | medium | [[Agent Robustness]] |

边界：这些 claim 当前主要来自本地 extracted Page 1 / Abstract；没有读到 section/page/figure 时，不伪造页码或段落级证据。

## 方法 / 机制

- 核心方法：Library Drift 描述自演化 skill library 因无限积累和缺少 lifecycle management 而发生检索退化、错误注入和性能停滞。
...
- PDF：`assets/Library Drift - Diagnosing and Fixing a Silent Failure Mode in Self-Evolving LLM Skill Libraries.pdf`
- Extracted Markdown：`extracted/Library Drift - Diagnosing and Fixing a Silent Failure Mode in Self-Evolving LLM Skill Libraries.extracted.md`
...
|---|---|---|---|
| Library drift | 很可能值得建卡，需和 [[Tool Registry]]、[[Agent Robustness]]、memory drift 边界区分。 | extracted Page 1 / Abstract | P1 |
| Skill lifecycle management | 可作为 workflow / tool governance topic 候选。 | extracted Page 1 / Abstract | P1 |

## 我的疑问

- Hermes / OMX 的 skill registry 是否有 active cap、retirement、contribution score？
- trace-level diagnostics 如何避免把短期任务失败误判成 skill 失效？
...
- 摘录入口：[[Library Drift - Diagnosing and Fixing a Silent Failure Mode in Self-Evolving LLM Skill Libraries.extracted#Page 1]]
- arXiv abstract：<https://arxiv.org/abs/2605.19576>
