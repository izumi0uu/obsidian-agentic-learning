...
pdf: assets/Nudging Beyond the Comfort Zone - Efficient Strategy-Guided Exploration for RLVR.pdf
extracted: extracted/Nudging Beyond the Comfort Zone - Efficient Strategy-Guided Exploration for RLVR.extracted.md
...
- 学科：cs.AI, cs.CL
- URL：<https://arxiv.org/abs/2605.15726>
- PDF：<https://arxiv.org/pdf/2605.15726v1>
- 本地 PDF：`assets/Nudging Beyond the Comfort Zone - Efficient Strategy-Guided Exploration for RLVR.pdf`（已本地保存；extracted 由 PDF 自动抽取）
- extracted：`extracted/Nudging Beyond the Comfort Zone - Efficient Strategy-Guided Exploration for RLVR.extracted.md`
- 阅读优先级：P2
...
这篇服务 reasoning / RLVR 背景：verifiable reward 不只需要更多采样，还需要把模型推向 hard cases 和新策略。

## 一句话

它研究 RLVR 中的 strategy-guided exploration，目标是让 reasoning model 走出已有采样轨迹的舒适区。
...
- 原文短摘：
  > Reinforcement learning with verifiable rewards (RLVR) has emerged as a scalable paradigm for improving the reasoning capabilities of large language models.
- 中文概括：
...
- 原文短摘：
  > However, its effectiveness is fundamentally limited by exploration: the policy can only improve on trajectories it has already sampled.
- 中文概括：
...
- 原文短摘：
  > While increasing the number of rollouts alleviates this issue, such brute-force scaling is computationally expensive, and existing approaches that modify the optimization objective provide...
- 中文概括：
...
|---|---|---|---|
| RLVR 的改进受限于探索质量，不只是 reward 是否可验证。 | Abstract / motivation | medium | [[Reasoning Trace]] |
| 策略引导可能帮助模型覆盖更难的 reasoning trajectories。 | Method | medium | [[Trajectory]] |

边界：没有读到 section/page/figure 时，先写 source note 小节，不要伪造页码或段落级证据。

## 方法 / 机制

- 核心方法：从摘要看，论文围绕 `Nudging Beyond the Comfort Zone: Efficient Strategy-Guided Exploration for RLVR` 提出一个前沿方向；具体机制待精读 Method / Framework。
...
- PDF：`assets/Nudging Beyond the Comfort Zone - Efficient Strategy-Guided Exploration for RLVR.pdf`
- Extracted Markdown：`extracted/Nudging Beyond the Comfort Zone - Efficient Strategy-Guided Exploration for RLVR.extracted.md`
...
|---|---|---|---|
| RLVR exploration | reasoning 训练子问题，先作为 source 候选。 | Abstract / source note | P2 |
| [[Reasoning Trace]] | 训练和评测都依赖可观察推理轨迹。 | Abstract / source note | P2 |

## 我的疑问

- strategy 是人工定义、模型生成还是从失败样本中学习？
- 它和 GRPO / rejection sampling 的关系是什么？
- 可验证 reward 是否覆盖真实 Agent 任务？
...
- 摘录入口：[[Nudging Beyond the Comfort Zone - Efficient Strategy-Guided Exploration for RLVR.extracted#Page 1]]
- arXiv abstract：<https://arxiv.org/abs/2605.15726>
