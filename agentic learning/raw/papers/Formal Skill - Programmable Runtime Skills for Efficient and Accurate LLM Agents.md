...
pdf: assets/Formal Skill - Programmable Runtime Skills for Efficient and Accurate LLM Agents.pdf
extracted: extracted/Formal Skill - Programmable Runtime Skills for Efficient and Accurate LLM Agents.extracted.md
arxiv: https://arxiv.org/abs/2605.19604
doi: 10.48550/arXiv.2605.19604
author:
  - Xi Zhang
  - Meijun Gao
  - Yuntian Zhao
...
conflicts: []
status: seed
source: https://arxiv.org/abs/2605.19604
related:
  - "[[Tool Use]]"
  - "[[Tool Calling]]"
  - "[[Tool Registry]]"
...
- 本地 PDF：`assets/Formal Skill - Programmable Runtime Skills for Efficient and Accurate LLM Agents.pdf`（已本地保存；extracted 由 PDF 自动抽取）
- extracted：`extracted/Formal Skill - Programmable Runtime Skills for Efficient and Accurate LLM Agents.extracted.md`
- 阅读优先级：P1-A
...
这篇和 Hermes / OMX skill 概念高度相关：它把 skill 从 Markdown 指令包推进到 runtime-native object，强调 metadata、schema、executor、hook policy 和 skill-local state。

## 一句话

Formal Skill 把可复用能力做成可执行、可路由、可观察、带状态和 hook policy 的 runtime object，而不是长 prompt 文档。
...
#### 必读块 1：metadata + schema

- 位置：extracted Page 1 / Abstract（必要时连同 Introduction 交叉核对）
- 为什么必读：说明 skill 不只是自然语言说明，而有结构化入口。
- 原文短摘：
  > JSON metadata and action schemas, reliable Python executors
- 中文概括：
  - 这部分提供第一轮 source-level 证据，帮助判断论文主张属于问题定义、方法机制、评测协议还是应用场景。
  - metadata 如何影响 discovery / routing，schema 如何约束 action。
...
- 位置：extracted Page 1 / Abstract（必要时连同 Introduction 交叉核对）
- 为什么必读：这里是 skill 与 tool/workflow/macro 的边界。
- 原文短摘：
  > reliable Python executors, hook-governed control logic
- 中文概括：
  - 这部分提供第一轮 source-level 证据，帮助判断论文主张属于问题定义、方法机制、评测协议还是应用场景。
  - 哪些逻辑应该进入 executor，哪些进入 host policy 或 workflow runtime。
  - 精读时需要回到正文 section、figure、table 和 limitations，不能只凭摘要外推成稳定工程标准。
- 我需要理解的机制：
  1. 论文对象、输入输出、运行时组件和验证边界分别是什么。
  2. 机制收益来自模型能力、系统结构、数据组织、训练策略还是评测协议。
...
- 位置：extracted Page 1 / Abstract（必要时连同 Introduction 交叉核对）
- 为什么必读：证明作者把 skill 当作 runtime control surface。
- 原文短摘：
  > executable state machines and hook policies
- 中文概括：
  - 这部分提供第一轮 source-level 证据，帮助判断论文主张属于问题定义、方法机制、评测协议还是应用场景。
  - skill-local state 与全局 Agent State / Memory 的边界。
  - 精读时需要回到正文 section、figure、table 和 limitations，不能只凭摘要外推成稳定工程标准。
- 我需要理解的机制：
  1. 论文对象、输入输出、运行时组件和验证边界分别是什么。
  2. 机制收益来自模型能力、系统结构、数据组织、训练策略还是评测协议。
...
- 可能更新的概念卡：[[Tool Use]]、[[Tool Calling]]、[[Tool Registry]]、[[Agent Harness]]

## 论文主张

| Claim | Evidence anchor | Confidence | Target concept |
|---|---|---|---|
| Formal Skill 将 reusable procedure 从 prompt 文本移到 executable runtime object。 | extracted Page 1 / Abstract | high | [[Agent Harness]] |
| skill 的可靠性来自 schema、executor、hook policy 和 local state，而不只是模型记住步骤。 | extracted Page 1 / Abstract | medium | [[Workflow Guardrails]] |

边界：这些 claim 当前主要来自本地 extracted Page 1 / Abstract；没有读到 section/page/figure 时，不伪造页码或段落级证据。

## 方法 / 机制

- 核心方法：Formal Skill 把可复用能力做成可执行、可路由、可观察、带状态和 hook policy 的 runtime object，而不是长 prompt 文档。
...
- PDF：`assets/Formal Skill - Programmable Runtime Skills for Efficient and Accurate LLM Agents.pdf`
- Extracted Markdown：`extracted/Formal Skill - Programmable Runtime Skills for Efficient and Accurate LLM Agents.extracted.md`
...
|---|---|---|---|
| Formal Skill | 可能值得建卡，但要先和 existing Tool Use / Tool Registry / OMX skill 语义对齐。 | extracted Page 1 / Abstract | P1 |
| Runtime skill | 可能是 skill/tool/workflow 边界的新主题。 | extracted Page 1 / Abstract | P1 |

## 我的疑问

- Hermes / OMX 的 skill 是否已经具备 runtime object 特征，还是仍主要是 instruction surface？
- Formal Skill 与 MCP tool、workflow、macro、agent hook 的最小区别是什么？
...
- 摘录入口：[[Formal Skill - Programmable Runtime Skills for Efficient and Accurate LLM Agents.extracted#Page 1]]
- arXiv abstract：<https://arxiv.org/abs/2605.19604>
