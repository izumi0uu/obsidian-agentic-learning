...
conflicts: []
status: seed
source: https://arxiv.org/abs/2605.15706
related:
...
- 论文标题：Differentiable Mixture-of-Agents Incentivizes Swarm Intelligence of Large Language Models
- 作者：Xingjian Wu, Junkai Lu, Siyu Yan, Xiangfei Qiu, Jilin Hu, Chenjuan Guo, Bin Yang
- 提交日期：2026-05-15
- 学科：cs.LG
- URL：<https://arxiv.org/abs/2605.15706>
- PDF：<https://arxiv.org/pdf/2605.15706v1>
- 本地 PDF：`assets/Differentiable Mixture-of-Agents Incentivizes Swarm Intelligence of Large Language Models.pdf`（已本地保存；extracted 由 PDF 自动抽取）
- extracted：`extracted/Differentiable Mixture-of-Agents Incentivizes Swarm Intelligence of Large Language Models.extracted.md`
- 阅读优先级：P2
...
这篇的启发点在动态 multi-agent topology：协作结构不固定，可能根据任务调整，但落地到工程系统前需要谨慎验证。

## 一句话

DMoA 探索可微的 Mixture-of-Agents，让 LLM agent 协作拓扑在推理或训练中更动态。
...
- 原文短摘：
  > Recent advances in Large Language Models (LLMs) have catalyzed the development of multi-agent systems (MAS) for complex reasoning tasks.
- 中文概括：
...
- 原文短摘：
  > However, existing MAS typically rely on pre-defined or pre-compiled communication topologies, which limits their flexibility and adaptability to dynamic task requirements.
- 中文概括：
...
|---|---|---|---|
| 固定 planner / critic / executor 拓扑可能不是多 Agent 协作的唯一形式。 | Abstract / method | medium | [[Multi-agent Orchestration]] |
| 动态协作结构需要证明收益超过成本和复杂度。 | Evaluation | medium | [[Evaluation]] |

边界：没有读到 section/page/figure 时，先写 source note 小节，不要伪造页码或段落级证据。

## 方法 / 机制

- 核心方法：从摘要看，论文围绕 `Differentiable Mixture-of-Agents Incentivizes Swarm Intelligence of Large Language Models` 提出一个前沿方向；具体机制待精读 Method / Framework。
...
- PDF：`assets/Differentiable Mixture-of-Agents Incentivizes Swarm Intelligence of Large Language Models.pdf`
- Extracted Markdown：`extracted/Differentiable Mixture-of-Agents Incentivizes Swarm Intelligence of Large Language Models.extracted.md`
...
|---|---|---|---|
| Mixture-of-Agents | 可能是多 Agent routing 变体，先不直接建卡。 | Abstract / source note | P2 |
| [[Multi-agent Orchestration]] | 动态协作拓扑的工程边界。 | Abstract / source note | P2 |

## 我的疑问

- 可微训练部分和实际推理系统之间如何衔接？
- 动态拓扑是否可解释、可调试、可复现？
- 和普通 ensemble / router 的差异是什么？
...
- 摘录入口：[[Differentiable Mixture-of-Agents Incentivizes Swarm Intelligence of Large Language Models.extracted#Page 1]]
- arXiv abstract：<https://arxiv.org/abs/2605.15706>
