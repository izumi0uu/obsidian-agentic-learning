...
pdf: assets/AutoResearchClaw - Self-Reinforcing Autonomous Research with Human-AI Collaboration.pdf
extracted: extracted/AutoResearchClaw - Self-Reinforcing Autonomous Research with Human-AI Collaboration.extracted.md
...
- 本地 PDF：`assets/AutoResearchClaw - Self-Reinforcing Autonomous Research with Human-AI Collaboration.pdf`（已本地保存；extracted 由 PDF 自动抽取）
- extracted：`extracted/AutoResearchClaw - Self-Reinforcing Autonomous Research with Human-AI Collaboration.extracted.md`
- 阅读优先级：P2
...
这篇是 autonomous research agent 系统论文，适合快速扫框架图和失败恢复机制。录入价值在 research pipeline、human intervention modes、verifiable reporting 和跨运行经验积累。

## 一句话

AutoResearchClaw 把自主研究做成 multi-agent debate、self-healing executor、verifiable reporting、HITL collaboration 和 cross-run evolution 的组合 pipeline。
...
#### 必读块 2：Pivot/Refine loop

- 位置：extracted Page 1 / Abstract（必要时连同 Introduction 交叉核对）
- 为什么必读：这是失败恢复机制，适合对比 Hermes 的任务失败处理。
- 原文短摘：
  > self-healing executor with a Pivot/Refine decision loop
- 中文概括：
  - 这部分提供第一轮 source-level 证据，帮助判断论文主张属于问题定义、方法机制、评测协议还是应用场景。
  - 失败是变成信息，还是只是重试；什么时候 pivot，什么时候 refine。
  - 精读时需要回到正文 section、figure、table 和 limitations，不能只凭摘要外推成稳定工程标准。
- 我需要理解的机制：
  1. 论文对象、输入输出、运行时组件和验证边界分别是什么。
  2. 机制收益来自模型能力、系统结构、数据组织、训练策略还是评测协议。
...
#### 必读块 3：verifiable reporting

- 位置：extracted Page 1 / Abstract（必要时连同 Introduction 交叉核对）
- 为什么必读：这是 research agent 与普通写作 agent 的安全边界。
- 原文短摘：
  > verifiable result reporting that prevents fabricated numbers and hallucinated citations
- 中文概括：
  - 这部分提供第一轮 source-level 证据，帮助判断论文主张属于问题定义、方法机制、评测协议还是应用场景。
  - 结果数字、引用、实验产物如何被 verifier 约束。
  - 精读时需要回到正文 section、figure、table 和 limitations，不能只凭摘要外推成稳定工程标准。
- 我需要理解的机制：
  1. 论文对象、输入输出、运行时组件和验证边界分别是什么。
  2. 机制收益来自模型能力、系统结构、数据组织、训练策略还是评测协议。
...
- 可能更新的概念卡：[[Agent Harness]]、[[Multi-agent Orchestration]]、[[Human-in-the-loop]]、[[Evaluation]]

## 论文主张

| Claim | Evidence anchor | Confidence | Target concept |
|---|---|---|---|
| autonomous research agent 需要把失败实验转成可复用信息，而不是线性 pipeline 失败即停。 | extracted Page 1 / Abstract | medium | [[Agent Harness]] |
| research pipeline 需要 verifiable reporting 防止编造数字和引用。 | extracted Page 1 / Abstract | high | [[Evaluation]] |

边界：这些 claim 当前主要来自本地 extracted Page 1 / Abstract；没有读到 section/page/figure 时，不伪造页码或段落级证据。

## 方法 / 机制

- 核心方法：AutoResearchClaw 把自主研究做成 multi-agent debate、self-healing executor、verifiable reporting、HITL collaboration 和 cross-run evolution 的组合 pipeline。
...
- PDF：`assets/AutoResearchClaw - Self-Reinforcing Autonomous Research with Human-AI Collaboration.pdf`
- Extracted Markdown：`extracted/AutoResearchClaw - Self-Reinforcing Autonomous Research with Human-AI Collaboration.extracted.md`
...
|---|---|---|---|
| Autonomous research agent pipeline | 可先作为 research-agent topic 候选，不急建卡。 | extracted Page 1 / Abstract | P2 |
| Human intervention mode | 可和 [[Human-in-the-loop]] 合并更新。 | extracted Page 1 / Abstract | P2 |

## 我的疑问

- 这篇的 ARC-Bench 是否可复现，还是主要服务论文系统评估？
- human intervention 的七种模式能否迁移到 Hermes 的人工审批/接管点？
...
- 摘录入口：[[AutoResearchClaw - Self-Reinforcing Autonomous Research with Human-AI Collaboration.extracted#Page 1]]
