...
- 为什么必读：定位论文的核心证据。
- 原文短摘：
  > To narrow this gap, we introducePersona Policies( PPol), a plug-and-play control layer that induces realistic behavioral variation in user simulators while preserving the original task goals.
- 中文概括：
...
- 为什么必读：定位论文的核心证据。
- 原文短摘：
  > Given a task, program G generates N persona policies that vary in how simulated users communicate.
- 中文概括：
  - 这段原文直接支撑本块阅读目标：定位论文的核心证据。
  - 当前摘录只证明作者在摘要或第一页如何界定问题、机制或结果；后续写稳定概念卡时仍要回正文核对方法、实验设置和限制。
- 我需要理解的机制：
...
- 为什么必读：定位论文的核心证据。
- 原文短摘：
  > Rather than hand-crafting personas, we cast persona generation as an LLM-driven evolutionary program search that optimizes a Python generator to discover behaviors and translate them into task-preserving roleplaypolicies.
- 中文概括：
  - 这段原文直接支撑本块阅读目标：定位论文的核心证据。
  - 当前摘录只证明作者在摘要或第一页如何界定问题、机制或结果；后续写稳定概念卡时仍要回正文核对方法、实验设置和限制。
- 我需要理解的机制：
...
- 核心方法：从本地 extracted Page 1 / Abstract 可确认，论文核心机制与「Persona Policies」相关：Given a task, program G generates N persona policies that vary in how simulated users communicate.
...
- 和相邻方法的差别：当前 Abstract 支持的差别线索是「simulator bias / Persona Policies / evolutionary program search」；不要把标题术语直接升格为稳定概念。

## 实验 / 证据

- 数据集 / benchmark：从本地 extracted Page 1 / Abstract 可确认的证据线索：Rather than hand-crafting personas, we cast persona generation as an LLM-driven evolutionary program search that optimizes a Python generator to discover behaviors and translate them into task-preserving roleplaypolicies.
