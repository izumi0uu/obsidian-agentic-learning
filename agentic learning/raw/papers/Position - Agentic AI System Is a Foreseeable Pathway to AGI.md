...
- 为什么必读：定位论文的核心证据。
- 原文短摘：
  > This paper challenges the dogma that purely scaling a single model is sufficient to achieve Artificial General Intelligence.
- 中文概括：
...
- 为什么必读：定位论文的核心证据。
- 原文短摘：
  > In this work, we present a series of demonstrations and theoretical derivations to substantiate the claim thatAgentic AI is the foreseeable cross-level move towards AGI.
- 中文概括：
  - 这段原文直接支撑本块阅读目标：定位论文的核心证据。
  - 当前摘录只证明作者在摘要或第一页如何界定问题、机制或结果；后续写稳定概念卡时仍要回正文核对方法、实验设置和限制。
- 我需要理解的机制：
...
- 核心方法：从本地 extracted Page 1 / Abstract 可确认，论文核心机制与「agentic system claim」相关：In this work, we present a series of demonstrations and theoretical derivations to substantiate the claim thatAgentic AI is the foreseeable cross-level move towards AGI.
...
- 和相邻方法的差别：当前 Abstract 支持的差别线索是「scaling challenge / agentic system claim / topology progression」；不要把标题术语直接升格为稳定概念。

## 实验 / 证据

- 数据集 / benchmark：从本地 extracted Page 1 / Abstract 可确认的证据线索：We demonstrate that Agentic AI achieves exponentially superior generalization and sample efficiency.
