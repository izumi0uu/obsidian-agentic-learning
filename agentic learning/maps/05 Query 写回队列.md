...
| P2 | done | [[Encoder-only]] / [[Decoder-only]] / [[Encoder-Decoder]] | 三者都来自 Transformer 架构变体，但可见性、训练目标和适用任务不同；其中最初先升格了 [[Decoder-only]]，后续再把另外两张卡补齐 | 已补入 [[LLM 基础结构对比]] 的“架构变体对比”小节，并新增 [[Encoder-only]]、[[Decoder-only]]、[[Encoder-Decoder]] 概念卡；三者现在都可作为稳定学习入口 |
