...
控制面的学习边界在于“跨运行实例的治理”，而不是某一次 agent loop 的推理细节。它通常把身份、权限、审批、调度、部署、trace、audit、成本和版本策略集中起来，让多个 agent / workflow 可以按组织规则运行。这样做的代价是会引入平台耦合：如果只是在本地写一个脚本，control plane 可能只是日志和配置；如果要多人协作、长期运行或处理敏感工具，它就会变成生产化必需层。
## 它解决什么问题
...
- Evidence type: official framework docs + framework comparison map + engineering synthesis.
- Boundary: 证据支持的是平台治理职责集合；不同框架对 control plane / runtime / console 的命名不同，不应把某个产品菜单等同为通用定义。
## 复习触发
