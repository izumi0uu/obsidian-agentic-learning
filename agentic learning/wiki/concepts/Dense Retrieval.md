...
很多 dense retriever 的具体实现是 bi-encoder / dual-encoder / 双塔结构：query 和 chunk 各自独立编码成向量，然后再做相似度比较。这样才能在召回阶段提前把 chunk 向量离线算好，在线只算 query 向量就能搜索；但它也意味着 query 和 chunk 在编码时没有逐词交互，所以在精细相关性判断上不如 [[Cross-Encoder]]。
...
和 Bi-encoder / 双塔的边界：bi-encoder 是常见实现结构；dense retrieval 是用这种结构做语义召回的路线。它们高度相关，但一个偏模型结构，一个偏检索策略。
