...
relations:
  ...
  - type: contrasts_with
    ...
    note: Dense Retrieval 常用 Bi-Encoder 把 query 和 chunk 分开编码，适合快速召回；Cross-Encoder 把二者放在一起判断，适合小候选集精排。
related:
  - "[[Retriever]]"
  - "[[Embedding]]"
  - "[[Bi-Encoder]]"
...
很多 dense retriever 的具体实现是 [[Bi-Encoder|bi-encoder / dual-encoder / 双塔结构]]：query 和 chunk 各自独立编码成向量，然后再做相似度比较。这样才能在召回阶段提前把 chunk 向量离线算好，在线只算 query 向量就能搜索；但它也意味着 query 和 chunk 在编码时没有逐词交互，所以在精细相关性判断上不如 [[Cross-Encoder]]。

相似度计算还会牵涉一个容易忽略的几何细节：有些系统会对 query 和 chunk embedding 做 [[L2 Normalization]]，再用 cosine similarity 或 dot product 排序。两边都归一化后，dot product 等价于 cosine；如果没有归一化，dot product 会同时受到向量方向和长度影响。
...
当向量数量很大时，Dense Retrieval 通常不会在线暴力比较每个向量，而是交给向量索引做 [[Approximate Nearest Neighbor Search|ANN]]。[[HNSW]] 是常见的图索引实现思路。这里要分清：Dense Retrieval 是语义向量召回路线；ANN / HNSW 是让这条路线在大规模数据上跑得动的索引机制。
...
和 [[Bi-Encoder]] / 双塔结构的边界：Bi-Encoder 是常见模型结构；Dense Retrieval 是用这种结构做语义召回的路线。它们高度相关，但一个偏模型结构，一个偏检索策略。

和 [[L2 Normalization]] 的边界：L2 normalization 是向量相似度计算前的尺度处理；它不是新的召回路线，也不替代 chunking、hybrid search 或 reranking。

和 [[Semantic Search]] 的边界：semantic search 是任务目标，强调按语义找资料；Dense Retrieval 是实现 semantic search 的常见向量路线。

和 [[Approximate Nearest Neighbor Search]] / [[HNSW]] 的边界：Dense Retrieval 定义“用 dense embedding 召回”；ANN / HNSW 定义“大规模向量怎样近似快速搜索”。

## 现代性状态

- 判定：foundation / current-practice。
- 稳定部分：RAG 常用稠密向量召回语义相似 chunk。
- 易变部分：embedding 模型、[[Vector Similarity Metrics|相似度度量]] 默认值、向量索引、ANN 算法、reranker 组合和产品能力变化快。
...
- Evidence type: RAG interview notes + retrieval concept synthesis.
- Boundary: Dense Retrieval 指基于 embedding 的语义相似候选召回，不等于向量数据库本身、cross-encoder reranking、BM25 或最终答案生成。
## 复习触发
