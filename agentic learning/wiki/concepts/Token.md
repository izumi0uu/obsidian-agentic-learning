...
LLM 生成文本时，本质上是在当前上下文中预测下一个 token。人看到的是句子、段落和代码，模型内部先通过 tokenizer 把输入切成 token ID，再通过 [[Token Embedding]] 把这些 ID 变成向量表示并进入 Transformer。[[LLM]] 卡已经强调 LLM 根据上下文生成 token 序列；[[Attention Is All You Need]] 作为 Transformer 地基，讨论的是序列位置、embedding、attention 等机制，这些都建立在离散序列单位之上。
...
和 [[Token Embedding]] 的边界：token / token ID 是离散单位；token embedding 是模型内部查表得到的连续向量。
