...
从算法层看，NLP 更像一个方法药箱，而不是一把固定工具。简单任务可以用计数、规则、正则或词典；中等难度任务可以用传统机器学习、序列标注、向量相似度或小型分类器；更高难度的理解、生成、推理和跨任务迁移可以交给 [[LLM]]。所以问“这个系统用了什么 NLP”，真正要追问的是：它把哪些自然语言子任务拆出来了，每个子任务用了规则、统计模型、向量模型、深度学习模型还是 LLM。
...
- 方法编排：针对不同语言子任务选择规则、统计模型、向量相似度、小模型或 LLM，而不是默认一招打到底。

## 它不是什么

NLP 不是 [[LLM]]。LLM 可以处理很多 NLP 任务，但 NLP 是更宽的任务域，历史上也包括规则系统、统计模型、BERT 类模型、检索排序模型和专用分类器。

NLP 不是某一个算法。它是一组自然语言任务、算法、模型和工程方法的集合；一个真实 NLP pipeline 往往会把分词、分类、抽取、相似度、生成和评估拆成不同模块。
...
- 把 NLP 当成单一算法：NLP 更像任务域加方法集合，里面可能同时有规则、统计、向量和神经网络模型。
...
一个小但有启发的边界：NLP 任务常常不要求“最聪明的模型”，而要求“最合适的求解器”。字数统计、句子切分、关键词过滤这类任务用规则就够；语义近似、意图识别、开放式摘要和报告生成才更需要 embedding、分类模型或 LLM。把任务难度拆开，能同时降低成本、延迟和不可解释风险。
