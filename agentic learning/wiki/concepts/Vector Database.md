...
从 embedding lesson 的角度看，向量库的关键不是“能存一列数组”，而是能把 [[Embedding]] 产生的 dense vectors 按 [[Vector Similarity Metrics]] 搜索，并用 [[Approximate Nearest Neighbor Search|ANN]] / [[HNSW]] 这类索引避免全量扫描。更大规模时，还会考虑 [[Matryoshka Embeddings]] 的维度截短、[[Embedding Quantization]] 的低精度压缩，以及 full-precision rescore / reranker 的二阶段质量控制。
...
和 [[Approximate Nearest Neighbor Search]] / [[HNSW]] 的边界：vector database 是基础设施产品或系统层；ANN / HNSW 是其中可能使用的索引和查询算法。

和 [[Vector Similarity Metrics]] 的边界：metric 决定向量之间怎样算近；vector database 负责按这个 metric 建索引和查询。

和 [[Embedding Quantization]] / [[Matryoshka Embeddings]] 的边界：它们是向量存储与检索成本优化手段，不等同于数据库本身；使用后仍要评估召回质量。
...
现代系统把 vector database 放在 retrieval infrastructure 层，并配套 ingestion、metadata schema、权限同步、删除更新、trace 和 RAG eval。它是 RAG 的底座之一，不是质量保证本身。真正上线前还要验证 metric / normalization、ANN recall、filter 行为、量化损失、索引重建和 rerank 后的端到端效果。
...
- Anchor: [[AI Engineering From Scratch - Embeddings#关键事实]]
