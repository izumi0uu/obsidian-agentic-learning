...
[[Smithery MCP Registry]] 可以作为第三方 MCP registry 的具体例子来读：用户侧是发现和连接 MCP server，开发者侧是把 server 发布到 Smithery registry，API / registry 文档则提供按关键词、tag、remote / verified 等元数据筛选 server 的入口。

这说明 MCP registry 的稳定学习价值是 server discovery、publishing、metadata filtering 和 connection onboarding。它不应该被读成“协议语义的一部分”，也不应该被读成“上架即可信”。Smithery 或任何 registry 返回的 server 都还要经过 host 侧的来源审查、版本锁定、权限收窄、sandbox、approval 和 trace。
...
- Evidence type: source evidence — [[Agent 工程基础设施主源#为什么收]]；[[Model Context Protocol 官方文档#为什么收]]；[[Smithery MCP Registry#为什么收]]；[[Smithery MCP Registry#关键事实]]
- Evidence type: source boundary — 本卡只使用现有 source note / project note 的小节级证据；未伪造段落、页码或不存在的小节。
- Evidence type: engineering synthesis — “概念详解”“边界细节”“现代性状态”把 [[Agent 工程基础设施主源]]；[[Model Context Protocol 官方文档]]；[[Smithery MCP Registry]] 与本 vault 的 Agent 工程学习目标综合起来。
