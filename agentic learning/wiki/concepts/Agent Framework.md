...
evidence:
  ...
  - "[[Agent Framework 全量选型对比 2026-05#证据锚点]]"
related:
  - "[[Agent]]"
  - "[[Agent Harness]]"
  - "[[Tool Calling]]"
...
## 2026-05-12 热门框架全量对比补充

本卡的“代表生态”已通过 [[Agent Framework 全量选型对比 2026-05]] 扩展为 13 个官方来源支持的框架/SDK/toolkit 横向比较。新增边界是：热门工具并不处在同一层，不能只问“哪个框架最强”。应先判断它主要接管哪类责任：state graph、provider SDK、enterprise workflow、conversation team、crew/flow、data/RAG、typed Python、platform stack、TypeScript workflow、frontend UI toolkit、Google Cloud ADK、message platform，还是 role-playing society。

工程综合：当框架同时覆盖工具、状态、workflow、多 Agent、memory/RAG、HITL、trace/eval 和部署时，它更像完整 Agent framework / platform；当它主要服务模型 provider、typed output 或 UI streaming 时，它更像 SDK / toolkit，常需要外接 runtime、harness 或平台治理。
