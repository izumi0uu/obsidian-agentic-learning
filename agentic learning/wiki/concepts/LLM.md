...
从宏观上看，LLM 可以被看成一个巨大的自回归概率生成算法：输入 prompt / context，模型逐步预测下一个 [[Token]] 的概率分布，再通过解码策略把概率分布变成实际输出。从微观上看，LLM 不是一个简单公式，而是一组模块和算法共同工作：tokenization 把文本转成 token ID，embedding 和位置表示把 token 放进向量空间，[[Self-Attention]] / [[Multi-Head Attention]] 建模上下文关系，前馈网络、归一化和激活函数改变表示，最后由采样或搜索策略决定输出哪个 token。
