...
source:
  ...
  - "[[167 01_AI 06_评测与监控 为什么 BLEU ROUGE 评 LLM 不够？]]"
  - "[[170 01_AI 06_评测与监控 为什么传统的 NLP 评估指标（如 BLEU, ROUGE）对于评估现代 LLM 的生成质量来说，存在很大的局限性？]]"
...
evidence:
  ...
  - "[[167 01_AI 06_评测与监控 为什么 BLEU ROUGE 评 LLM 不够？#题目正文]]"
  - "[[170 01_AI 06_评测与监控 为什么传统的 NLP 评估指标（如 BLEU, ROUGE）对于评估现代 LLM 的生成质量来说，存在很大的局限性？#题目正文]]"
...
其中“自动指标”这一层，最容易被初学者误解成“有一个分数就够了”。传统的 BLEU 和 ROUGE 主要看候选答案与参考答案的字面重合，因此更贴近早期机器翻译、摘要等有相对稳定参考答案的任务。它们适合回答“写法像不像参考答案”“摘要有没有覆盖参考内容”，却不擅长回答“语义是否等价”“事实是否准确”“推理是否成立”“回答是否安全”。这也是为什么它们在现代 LLM 场景里经常显得不够。

更具体地说，BLEU 更偏字面精确匹配，可以把它理解成“生成文本和标准答案在 n-gram 上有多像”；ROUGE 更常用于摘要，通常更偏内容覆盖，可以把它理解成“参考答案里的关键内容，你有没有覆盖到”。这两个指标都不是错，只是它们主要度量“词面重合”，而现代 LLM 的很多输出存在大量“换种说法但依然正确”的情况。于是模型明明答对了，BLEU / ROUGE 也可能打低分。

BERTScore 代表的是另一条思路：不只看字面，而是借助 BERT 这类上下文化编码模型比较语义相似度。这里有一个常见混淆要切开：BERT 是模型家族，本身不是评估指标；BERTScore 才是利用这类模型表示做出来的自动评估指标。它比 BLEU / ROUGE 更能识别“措辞不同但语义接近”的回答，因此更接近现代开放式生成任务的需求；但它依然不是完整答案，因为语义相近不自动等于事实正确、推理可靠或业务上可用。

所以更稳的边界是：自动指标只能覆盖 Evaluation 的一部分。BLEU / ROUGE 适合帮助理解传统词面重合指标的价值与局限；BERTScore 适合帮助理解“从词面匹配走向语义匹配”的升级；而 BERT 本身更适合放在 [[NLP]] 和 LLM 演进语境里理解，而不是误记成一个评估指标。现代 LLM / Agent 评测一般需要把自动指标、规则校验、任务成功率、[[LLM-as-Judge]] 和人工评估组合起来看。
...
- 误解：BLEU / ROUGE 分数低，就一定说明回答质量差。风险是把“措辞不同但语义正确”的开放式回答误判为差答案。
- 误解：BERTScore 更语义化，就能替代事实性、推理性和安全性评估。风险是把“语义像”误当成“内容真的对”。
...
- BLEU、ROUGE、BERTScore 都属于自动指标；它们回答的是“文本和参考答案在某种相似性定义下有多接近”，不直接回答任务是否真正完成。
...
- Evidence type: interview raw notes on BLEU / ROUGE limitations — [[167 01_AI 06_评测与监控 为什么 BLEU ROUGE 评 LLM 不够？#题目正文]] and [[170 01_AI 06_评测与监控 为什么传统的 NLP 评估指标（如 BLEU, ROUGE）对于评估现代 LLM 的生成质量来说，存在很大的局限性？#题目正文]] support the “lexical-overlap metrics are insufficient for modern open-ended generation” boundary.
- Confidence: medium
- Boundary: GAIA/SWE-bench 支持“真实任务/patch/tests”评估思路；LangSmith/Langfuse 支持 trace/dataset/evaluator/score/monitoring 的工程闭环；BLEU / ROUGE 相关 raw notes 支持“传统词面重合指标不足以覆盖现代 LLM 生成质量”的学习边界；“四层 Evaluation 结构”是本 vault 的工程综合理解。

## 复习触发

- 为什么单个 benchmark 分数不足以代表 evaluation？
- 为什么 BLEU / ROUGE 不足以代表现代 LLM 的生成质量？
- BERT、BERTScore 和 BLEU / ROUGE 分别属于“模型”还是“评估指标”？
