...
evidence:
  ...
  - "[[raw/repos/xiaolinnote/questions/031 ai rag 18. 怎么量化你的 RAG 效果？#第一层：检索层评估]]"
  - "[[raw/repos/xiaolinnote/questions/031 ai rag 18. 怎么量化你的 RAG 效果？#第二层：生成层评估（RAGAs 框架）]]"
...
常见指标家族里，检索层通常看 Hit@K / Hit Rate、Recall@K、MRR、nDCG@k 这类 ranking metrics，回答层通常看 Faithfulness / Groundedness 和 Answer Relevancy。前者回答“找没找到、排得靠不靠前”，后者回答“答案是否被上下文支撑、是否真的回答了用户的问题”。它们和 [[Context Recall]] / [[Context Precision]] 不一样：后两者更关注进入上下文的内容覆盖与噪音，而不是最终答案本身；它们也和 [[RAG Citation Faithfulness]] 不一样，后者更细地检查答案中的引用是否足以支持具体结论。
...
Embedding 模型评测要再切一层。[[MTEB]] 是 text embedding model 的公共 benchmark，适合做候选模型初筛；RAG Evaluation 评的是你的整条系统链路。一个模型 MTEB 分数高，不保证在你的 chunking、metadata、语言、权限、hybrid search、reranker 和上下文预算下最优。因此 embedding 选型应从 MTEB 进入业务 retrieval eval，而不是停在排行榜。
...
它也不是 [[MTEB]]。MTEB 主要评 embedding 模型的通用能力；RAG Evaluation 要评资料进入、检索、排序、上下文、引用、生成、权限、延迟和成本。
...
- Embedding benchmark boundary: [[AI Engineering From Scratch - Embeddings#关键事实]], [[AI Engineering From Scratch - Embeddings#边界提醒]]
