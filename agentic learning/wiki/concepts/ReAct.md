...
用户这张图可以作为 ReAct 工程分工的最小外部视角：[[LLM]] 负责生成下一步 reasoning/action，Tools 是可调用能力集合，Environment 是动作发生并返回状态变化的外部世界。真正的 loop 不在图中三个盒子之一，而是由外部框架或 [[Agent Harness]] 驱动。

![[react-tools-llm-environment.svg]]
...
- Asset: `agentic learning/raw/assets/react-tools-llm-environment.svg`（根据用户截图重绘，2026-05-10）
