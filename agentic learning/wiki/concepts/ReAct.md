...
和 [[ReWOO]] 的最小边界是 Observation 的回流位置：ReAct 让每次工具或环境返回的 Observation 进入下一轮 Thought / Action，适合需要动态修正路线的任务；ReWOO 则先规划 `Plan / #E` 证据槽，让 Worker 填 evidence，再由 Solver 汇总，适合证据需求可预先枚举、主要瓶颈是 prompt redundancy 的任务。这个边界提醒很重要：不是所有工具增强任务都需要完整 ReAct loop，但只要中间结果会改变下一步策略，就不能轻易移除 observation feedback。
