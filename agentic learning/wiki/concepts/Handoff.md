...
官方 SDK 或框架里，handoff 可能表现为 agent routing、handoff tool、graph edge、escalation 或 human approval node。社区讨论里也会把它说成“多个 agent 协作”，但学习时要避免人格化理解：真正重要的是控制权、上下文和权限如何迁移，以及 trace 里能不能看清这次迁移为什么发生。

在 runtime 上，handoff 可以是框架内的 agent 切换、workflow 节点跳转、人工接管，也可以跨系统进入协议层。协议如 [[A2A]] 关注跨 agent / 跨系统互操作；handoff 作为模式也可以完全发生在同一个进程、同一个框架或同一个团队工作流里。

在学习和工程实践中，handoff 还要回答“交出去后，原执行者是否仍负责”。有些 handoff 是完全转交，例如客服转给账单专员；有些是临时委托，例如主 Agent 调用专家 Agent 生成建议后仍由主 Agent 决策。两者都可能叫 handoff，但责任链和 trace 解释不同。
## 它解决什么问题
