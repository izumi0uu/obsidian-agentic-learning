...
evidence:
  ...
  - "[[AI Engineering From Scratch - Prompt Engineering#关键事实]]"
  - "[[AI Engineering From Scratch - Prompt Engineering#边界提醒]]"
  - "[[Prompt#证据锚点]]"
...
从模型机制看，提示词细化不应被写成训练语料检索。更准确地说，prompt 会进入 [[LLM]] 的上下文，模型在当前上下文条件下逐步预测下一个 [[Token|token]]。当 prompt 写明角色、领域、受众、格式、长度、包含项和排除项时，它不是在推理时访问外部训练样本库，而是在改变 `P(next token | context)` 的条件，让 [[Transformer]] 的 attention 层处理到更清晰的任务信号，并把合法输出空间收窄到更贴近任务的区域。

因此，提示词细化的价值不在于访问训练样本，而是减少模型需要猜的分支：泛泛地说“写一封营销邮件”，会留下受众、语气、长度、产品、证据和 CTA 等大量未定变量；写成“面向 B2B SaaS 工程经理、技术但不销售腔、150 词、包含一个指标、只给正文”，就把很多可能输出排除掉了。这个解释也保留一个风险边界：如果角色或任务具体到训练语料中几乎没有高质量模式，模型可能更容易生成自信但不可靠的内容。

可以把这层机制拆成几个原子判断：第一，prompt 细化增加条件，使模型在当前上下文下预测下一个 token，而不是从外部样本库召回答案；第二，角色、领域、受众、格式和禁止项都作为上下文 token 参与 Transformer 表示计算，改变生成时的语义邻域；第三，结构化格式、长度和排除条件会压低不合规序列的概率，让输出空间更窄；第四，few-shot 示例和 role prompt 更像推理时的临时任务定义，它们不更新参数，也不能替代证据、检索和评测。
...
- 误解：提示词细化等于训练数据检索。更准确地说，它是在推理时增加条件、收窄下一 token 概率分布和输出空间。
...
- [[AI Engineering From Scratch - Prompt Engineering#关键事实]] / [[AI Engineering From Scratch - Prompt Engineering#机制拆解（工程综合）]] / [[AI Engineering From Scratch - Prompt Engineering#边界提醒]]：支持 vague prompt 与 engineered prompt 的分布收窄比喻，并记录“不是训练数据检索”的机制边界。
