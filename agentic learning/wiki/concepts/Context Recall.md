...
工程上，Context Recall 常用于解释“答案为什么凭空猜”：模型可能会生成流畅答案，但关键 supporting chunk 根本没有进入上下文。排查时要先看 gold evidence 或人工标注证据是否出现在 retrieved/context set，再看生成阶段是否正确使用。它和 Context Precision 是一对张力：扩大 top-k 往往提高 recall，却可能降低 precision；缩短上下文能减少噪音，却可能漏掉必要证据。
## 它解决什么问题
...
- Boundary: Context Recall 只判断必要信息是否被覆盖，不判断这些信息是否靠前、是否被正确引用、也不直接等同普通“召回层”。
## 复习触发
