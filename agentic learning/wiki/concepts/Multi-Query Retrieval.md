...
用户问题和知识库文档之间不只存在同义词差异，还存在“提问角度”差异。用户问“产品多久能送到”，文档可能写“配送时效说明”；单个 query 的向量相似度或关键词命中都可能不够。Multi-Query Retrieval / Multi-Query Expansion（MQE）的策略是把一个问题扩展成 3～5 个不同角度的问法，每个 query 单独检索，再把结果合并去重。

它和 [[Query Rewrite]] 有重叠，但边界不同。Query Rewrite 是更大的检索前处理家族，可以做口语改写、补全上下文、[[HyDE]]、[[Step-back Prompting|step-back]]、多 query 等；Multi-Query Retrieval 特指“保留同一用户意图，但生成多个查询视角并并行检索”的策略。它也常作为 [[Multi-Route Retrieval]] 的一路：dense route、BM25 route、多 query route 分别补不同盲区。
...
和 [[Query Rewrite]] 的边界：Multi-Query Retrieval 是 query rewrite 家族里的一种扩展策略。它的 `up` 指向 [[Query Rewrite]]，因此会在 Abstract Folder 里作为 Query Rewrite 的子概念显示；这个层级只表示严格子策略，不表示所有查询增强策略都应该归到 Query Rewrite 下面。
...
和 [[HyDE]] / [[Step-back Prompting]] 的边界：HyDE 生成“像文档的假设答案”去检索；Step-back 生成更抽象的背景问题；Multi-Query Retrieval 生成多个同一意图下的查询视角。三者都能拓宽召回，但 query 形态和失败模式不同。
