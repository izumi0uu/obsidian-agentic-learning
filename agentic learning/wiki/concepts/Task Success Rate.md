...
和 [[Win Rate]] 的边界：Task Success Rate 是“任务是否成功”的绝对比例；Win Rate 是“版本 A 是否赢过版本 B”的相对比较比例。Win Rate 常需要人类或 [[LLM-as-Judge]] 做 pairwise 判断，适合开放式输出、偏好比较或数据生成质量评估；但它不能替代 hard success checker。两个版本都错时，A 仍可能因为更流畅而赢。

如果一个报告同时写 `success rate` 和 `win rate`，要追问：

- success 的 checker 是什么？
- win 的裁判是谁，rubric 是什么？
- tie 怎么算？
- A/B 样本是否同分布？
- 是否有人工校准或规则兜底？
...
- Win Rate 为什么不能直接当作 Task Success Rate？
