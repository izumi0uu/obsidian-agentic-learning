...
传统 workflow engine 早就有 durable execution 的思想；Agent 场景把它推到更明显的位置，是因为 LLM 调用具有不确定性、上下文窗口有限、工具副作用真实存在，而且 human-in-the-loop 可能让任务暂停很久。没有持久化边界时，Agent 只能把进度写在对话里，一旦进程或上下文丢失就难以安全恢复。

代表生态包括 Temporal、Restate、Inngest、LangGraph durable execution 等。它们的共同点不是都服务 Agent，而是都把长流程从脆弱的内存调用变成可恢复的运行记录。具体 API、graph 表达和 checkpoint store 属于实现层；概念层要抓住的是“状态、事件、副作用和恢复策略必须被 runtime 显式管理”。

对 Agent 学习尤其重要的一点是：durable execution 把“继续任务”从对话技巧变成系统能力。模型可以在恢复后重新阅读 state 和 event log，但它不需要凭空记住所有历史；runtime 才是保存事实进度的地方。这样也能把可重试错误、不可重试副作用和等待人工输入分开处理。
## 它解决什么问题
