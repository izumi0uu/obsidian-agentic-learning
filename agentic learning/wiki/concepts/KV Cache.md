...
KV Cache 也不是 Agent harness 层的 tool / retrieval / embedding cache。[[Agent Harness 缓存分层与命中率]] 讨论的是应用层和 serving 层缓存指标如何分层读取；KV Cache 本身仍是模型推理热路径里的 K/V 张量复用。
...
和 Agent harness 缓存命中率的边界：harness 通常不能直接决定某个内部 KV cache entry 是否命中；它能影响的是 prompt 前缀稳定性、上下文长度、模型/serving 选择、并发策略，以及 tool / retrieval / embedding / summary 这类应用层缓存的 key、TTL 和观测。
