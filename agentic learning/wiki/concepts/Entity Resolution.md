...
在 GraphRAG 和知识库场景里，Entity Resolution 是把“看起来像同一对象”的多处 mention 合成一个可追踪实体的过程。它需要证据和策略：别名、上下文、ID、时间、组织、来源可信度都可能参与判断。过度合并会把不同人/公司/概念混成一个节点，漏合并又会让图上关系分裂。现代系统通常把 resolution 决策、置信度和人工复核记录下来，因为错误合并会污染后续检索、统计和生成。
## 它解决什么问题
...
- Boundary: Entity Resolution 是实体合并决策，不是普通 keyword normalization，也不是构图后所有质量评估。
## 复习触发
