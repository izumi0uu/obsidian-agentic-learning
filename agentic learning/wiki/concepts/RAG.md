...
现代系统吸收 RAG 的方式，是把“知识更新”问题变成“可检索、可评估、可追溯的上下文治理”问题。它降低模型只靠参数胡编的概率，但新增了 ingestion 错误、过期资料、权限泄露、chunk 破碎、rerank 误排、引用不支持结论等失败点。所以 RAG 的学习重点不是记住缩写，而是能沿着 pipeline 定位：错在资料入库、检索、排序、上下文组织，还是生成阶段。
