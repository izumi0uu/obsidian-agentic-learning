...
Tool Calling 把“模型想使用外部能力”从自然语言意图变成结构化接口。早期 ReAct 风格系统常让模型输出 `Action: Search[query]`，再由程序解析文本；这种方式容易因为格式、拼写、参数缺失或注入内容而失败。Tool Calling 的核心改进，是先把工具暴露成 schema，让模型在候选工具和参数约束中生成 tool call，再由 runtime 校验、执行和回填结果。

它同时服务模型和工程系统。对模型来说，工具名、description、参数字段和枚举值提示“什么时候该用哪个工具”；对 runtime 来说，schema 是解析、类型校验、拒绝额外字段、生成 trace、触发 retry 和权限判断的入口。[[Toolformer]] 让我们看到 tool use 可以被看成模型能力；OpenAI、Anthropic 和 MCP 文档则把这个能力工程化成 tool / input schema / output schema / tool result 等接口对象。

边界上，Tool Calling 只接管“调用请求的表达”和一部分格式可靠性，不接管工具真实副作用。天气查询、文件写入、付款、发邮件、运行 shell 的风险完全不同，需要 [[Tool Permissioning]]、[[Approval Gate]]、sandbox、audit log 和业务校验。一个系统支持 tool calling API，并不等于它会可靠地选择工具、验证结果、保存状态或完成任务；这些属于 [[Agent Loop]] 和 [[Agent Harness]]。

现代系统里 Tool Calling 通常也是 tool governance 的入口。工具 description 决定模型什么时候想调用；schema 决定参数是否合法；runtime 决定是否真的执行；permission layer 决定是否需要审批；trace 决定失败后能否复现。把这些层拆开后，就能看清一个常见故障：模型生成了形式正确的 tool call，但业务上不该执行，或工具返回了看似成功但语义错误的结果。schema 只能降低接口歧义，不能替代目标判断和风险判断。
## 它解决什么问题
...
Tool Calling 也不等于 [[Constrained Decoding]]。前者是模型表达工具调用意图的接口层；后者是生成过程中按 schema / grammar mask 非法 token 的解码机制。strict tool arguments 可以用 constrained decoding 提高结构合法性，但工具选择、权限和业务正确性仍要由 runtime 处理。
...
## 现代系统怎么吸收 Tool Calling 的价值 / ReAct Action 的局限
...
- Anchor: [[OpenAI Function Calling 文档#Tool schema 锚点]], [[OpenAI Function Calling 文档#Strict mode / Structured Outputs]], [[OpenAI Structured Outputs 文档#Constrained decoding 锚点]], [[Anthropic Tool Use 文档#Tool schema 锚点]], [[Model Context Protocol 官方文档#Tool schema 补充]], [[raw/repos/xiaolinnote/questions/052 ai tools 1. 什么是 Function Calling ？原理是什么？#工具定义，schema 的每个字段都有含义]]
- Evidence type: paper/source note + official API/docs source notes + MCP spec source note + engineering synthesis.
- Confidence: high for schema/interface boundary; medium for vendor-specific API details because API fields and strict-mode behavior can change.
- Boundary: 本卡把 tool schema / tool call / tool execution / tool result 分开；安全、权限、业务合法性和最终任务成功不由 schema 单独保证。
## 复习触发
...
4. Tool Calling 和 Constrained Decoding 分别在接口层和解码层解决什么问题？
