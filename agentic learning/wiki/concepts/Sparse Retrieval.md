...
Sparse Retrieval 不是单一算法，而是一组围绕词项信号工作的机制。为了避免把表示、索引结构和打分函数混成同一种东西，可以分三层看：

| 层 | 作用 | 例子 | 边界 |
|---|---|---|---|
| 稀疏表示 / sparse representation | 把文本变成词项维度上的稀疏特征 | count vector、[[TF-IDF]]、sparse neural vector | 说明“文本怎样被表示”，不等于完整检索系统 |
| 检索结构 / search structure | 让系统快速找到包含某些词项的文档或 chunk | inverted index / 倒排索引 | 是索引结构，不是 BM25 或 TF-IDF 的同级打分算法 |
| 打分方式 / scoring | 对候选文档或 chunk 排序 | full-text scoring、[[BM25]] | 决定谁更靠前，不等于最终答案质量 |

`count vector` 是最基础的词频表示，适合放在 [[TF-IDF]] 的前置直觉里；`full-text scoring` 是全文检索里的泛化排序概念，暂不适合单独建卡；`inverted index` 是搜索系统核心结构，未来如果学习搜索/RAG 基础设施可以单独建卡；`sparse neural retrieval` 是现代扩展方向，需等 SPLADE / sparse embedding 等证据足够后再独立建卡。
