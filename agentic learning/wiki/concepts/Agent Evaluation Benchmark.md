...
还有一种 benchmark 证据很适合写进本卡正文，而不是误挂成子概念：scaffold ablation。DORA 在同一灾害响应 benchmark 上比较 ReAct、Plan-then-Execute、Reflexion 和 [[ReWOO]]，结果显示不同 scaffold 对同一任务分布的适配性差异。这里的学习点不是把 ReWOO 当 benchmark，而是看到 benchmark 可以暴露“移除 observation feedback 是否伤害任务”的边界。
