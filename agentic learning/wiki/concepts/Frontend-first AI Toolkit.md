...
Frontend-first 的价值在于把 AI 交互直接嵌入产品界面：流式输出、tool state、UI message、client/server 边界和用户反馈都成为一等对象。它适合快速构建聊天、assistant、表单协作和可视化工作流；但它通常不会单独承担底层 durable execution、跨服务权限治理或复杂 multi-agent 调度。学习时要把它看成应用层工具箱，而不是完整 agent platform。
## 它解决什么问题
...
- Evidence type: official Vercel AI SDK/docs + framework comparison map + engineering synthesis.
- Boundary: Frontend-first AI Toolkit 是产品 UI/应用层抽象，不等于 provider-first SDK、state graph runtime 或 backend control plane。
## 复习触发
