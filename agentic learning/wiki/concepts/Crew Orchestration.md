...
判断 crew 编排是否有价值，关键看角色之间是否真的有不同输入、工具、责任和验收标准。如果只是给同一个模型换几个角色名，系统会增加成本却不增加可靠性；如果研究、执行、审阅、审批、交付之间有清晰交接，crew 抽象可以把协作过程显式化。现代框架吸收它时，常会把 crew 与 workflow、state、human review、observability 放在一起，而不是只保留“多角色聊天”的外壳。
## 它解决什么问题
...
- Evidence type: official CrewAI/AutoGen docs + framework comparison map + engineering synthesis.
- Boundary: Crew Orchestration 是角色/任务协作结构，不等于任意 multi-agent chat，也不保证没有 central workflow 或 control plane。
## 复习触发
