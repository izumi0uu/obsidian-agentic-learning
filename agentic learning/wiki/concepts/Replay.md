...
Replay 通常依赖 [[Trace]] 或 [[Rollout|rollout record]]，但比 trace 要求更高。Trace 记录发生了什么；Replay 还需要足够的输入、工具返回、环境快照、随机种子或 mock，使系统能在可控条件下再次走过相同或相似路径。对 eval harness 来说，replay 是把线上失败转成 regression case 的桥：先保存失败样本，再固定变量，最后用新策略重跑，看是否真的修复。
...
- [[Rollout]]：保存一次执行 episode / run 的证据边界；Replay 使用其中足够完整的字段重放或重新评分。
