...
当 Agent 支持按需加载 skill 时，harness 还要管住“选择了什么做事方法”。`SKILL.md`、skill 描述和完成定义会影响模型选择能力包，但这些文本不应自动变成可信规则。harness 至少要检查：这个 skill 是否适用于当前任务，是否要求越权工具，是否扩大写入范围，是否和上级规则冲突，执行结果是否有外部证据。模型的 reflection 可以提出怀疑，但真正的 stop / replan / reject 应由 harness、registry、policy、trace 和 evaluation 共同承载。
## 它解决什么问题
