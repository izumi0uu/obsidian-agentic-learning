...
它把一次 [[Trajectory]] 或 [[Rollout]] 当作评价对象：工具顺序、观察读取、检索质量、权限边界、失败恢复、人工确认、成本、延迟、最终结果都可以进入 rubric。[[Trace]] 是主要证据来源，因为 trace 保存了模型调用、tool call、observation、error 和 score；但 trace 本身不评分，评分来自规则、代码 checker、LLM-as-judge、人工 review 或业务指标。
...
它也不等于 [[Rollout]]。Rollout 是一次实际运行及其证据单位；Trajectory Evaluation 是读取这些过程材料后给出判断。
...
2. [[Rollout]]：一次实际执行 episode / run，通常包含运行配置、终止状态和报告相关元数据。
3. [[Trace]]：系统记录下来的路径证据。
4. Evaluation：对这条路径或运行证据做判断的规则、rubric、judge 或人工 review。
