...
Prompt 重要，但它只是系统的一层。[[Prompt Engineering]] 把 prompt 当作可设计、可测试、可版本化的工程资产；[[Context Engineering]] 又把 prompt 扩展为运行时信息环境：不仅有文字指令，还有工具 schema、memory、RAG evidence、trace summary、权限规则和输出格式。Agent 系统里，prompt 常常与 state、tools、guardrails、evaluation 和 human-in-the-loop 配合；只靠“写得更好的 prompt”无法替代权限、检索、测试和审计。

证据边界：[[Prompt Engineering]] 支持把 prompt 设计、测试和版本化沉淀为工程实践；[[Plan-and-Solve Prompting]] 支持特定 prompt pattern 可以改善复杂推理；[[Context Engineering]] 支持现代系统把 prompt 放在更大的上下文装配问题中；OpenAI agent guide source note 支持构建 Agent 时需要清晰指令和边界。具体 prompt 模板是实践策略，不写成稳定定义。

Prompt 的学习重点不是“写一句神奇指令”，而是把任务、输入、约束、证据、输出格式和工具边界组织成模型可执行的上下文。它既可以是用户消息，也可以包含 system/developer 指令、few-shot 示例、retrieved evidence、tool schema 和安全限制。现代 Agent 系统会把 prompt 放进更大的 runtime：状态、工具、检索、评估和权限共同决定结果；prompt 很重要，但不应承担所有工程控制责任。
## 它解决什么问题
...
Prompt 也不是完整 [[Prompt Engineering]] 或 [[Context Engineering]]；前者是围绕 prompt 的工程实践，后者还包括信息选择、排序、预算、来源、记忆和工具结果装配。
...
和 [[Prompt Engineering]] 的边界：prompt 是输入内容，Prompt Engineering 是设计、测试、版本化和优化 prompt 的工程过程。

和 [[Context Engineering]] 的边界：Prompt Engineering 更关注指令文本、模板、示例和输出约束；Context Engineering 管理整轮模型输入环境。
...
- Boundary: Prompt 是一次模型调用的指令和上下文组织，不是模型权重、长期记忆、权限系统或事实校验器。
## 复习触发

1. Prompt、Prompt Engineering 和 Context Engineering 的区别是什么？
2. 为什么 prompt 不是安全边界？
