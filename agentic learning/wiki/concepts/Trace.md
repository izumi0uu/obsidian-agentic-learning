...
Trace 也不等于 [[Rollout]]。Rollout 偏一次实际执行 episode / run；trace 是记录这次执行的数据结构。一个足够完整的 trace 可以成为 rollout record 的核心材料，但 trace schema 不等于 rollout 的完整复现边界。
...
和 [[Rollout]] 的边界：rollout record 要能说明一次运行的任务、环境、动作、状态、失败和终止信息；trace 通常负责保存这些事件，但如果缺少环境配置、版本、预算或 dropped runs，后续仍可能无法重算 reported score。
