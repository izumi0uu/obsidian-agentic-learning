...
机制上，每个位置会根据自己生成 query，并和其他位置的 key 比较，得到权重后汇总 value。Transformer 原论文常用 [[Scaled Dot-Product Attention]] 作为这个比较和汇总的计算形式；decoder 侧会加 [[Masked Attention]] 来限制未来 token 可见性。这个过程让 token 表示能吸收上下文信息，所以模型可以处理指代、条件和跨句依赖。它仍然是一次前向计算中的关系建模，不会在计算结束后自动形成可查询、可审计的长期状态。

一个适合复述的直觉例子是：“我今天买了一本书，晚上才开始读它。”这里“它”的 query 会去匹配当前句子中其他 token 的 key；“书”的 key 更匹配，“我”或“晚上”不匹配。匹配后，“它”读取“书”的 value，于是当前 token representation 带上“书”的上下文信息。这个例子强调的是当前上下文内的关系更新，不是模型拥有长期记忆。
...
再换成视频里的例子：“我今天买了一本书，晚上才开始读它”。模型不是把“它”永久改写成“书”，而是在这一次前向计算里让“它”的表示吸收“书”的信息。
