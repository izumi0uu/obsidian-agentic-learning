...
evidence:
  ...
  - "[[Under the Hood of SKILL.md - Semantic Supply-chain Attacks on AI Agent Skill Registry#边界提醒]]"
related:
  - "[[MCP]]"
  - "[[Tool Registry]]"
  - "[[Prompt Injection]]"
...
Skill / `SKILL.md` 这类能力包说明也落在这个边界附近。[[Under the Hood of SKILL.md - Semantic Supply-chain Attacks on AI Agent Skill Registry]] 的 source note 支持一个更细的判断：skill 描述不是被动文档，而是会影响 Agent 发现、选择、信任和加载能力包的 operational text。因此错误或恶意 skill 不应只被看成“流程写错”，它可能是工具/能力供应链污染的一种形式。
...
- Agent 可能反思出 skill 不适用，但 reflection 不是 verification；真正的防线是 registry 审查、最小权限、trace、eval 和必要时的人类审批。

## 边界细节

Tool Poisoning 的边界在工具生态：description、schema、annotations、返回值和供应链都可能是攻击面。不能因为工具以 MCP/registry 形式出现就信任其文字说明。

对 skill-enabled Agent 来说，`SKILL.md` 里的适用条件、流程、完成定义和风险说明也会进入运行决策。一个 skill 被检索到或描述得很“适合”，不等于它可信、适用或安全；harness 仍要检查任务匹配、允许工具、可写范围、输出证据和失败回退。

## 现代性状态

frontier / volatile。Tool poisoning 随 MCP、registry、远程工具生态和 skill registry 快速演进。稳定问题是工具/能力元数据和返回值会进入模型上下文并影响行动。

## 证据锚点

- Evidence type: source evidence — [[MCP Tool Poisoning Threat Model#为什么收]]；[[OWASP Agentic Applications Top 10#为什么收]]；[[Under the Hood of SKILL.md - Semantic Supply-chain Attacks on AI Agent Skill Registry#需要我读的内容]]
- Evidence type: source boundary — 本卡只使用现有 source note / project note 的小节级证据；未伪造段落、页码或不存在的小节。
- Evidence type: engineering synthesis — “概念详解”“边界细节”“现代性状态”把 [[MCP Tool Poisoning Threat Model]]；[[OWASP Agentic Applications Top 10]]；[[Under the Hood of SKILL.md - Semantic Supply-chain Attacks on AI Agent Skill Registry]] 与本 vault 的 Agent 工程学习目标综合起来。
