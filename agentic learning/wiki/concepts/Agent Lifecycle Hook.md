...
官方 hooks 文档通常描述“哪些事件会触发、handler 接收什么输入、能返回什么控制结果”；observability 文档则描述“这些事件怎样变成 trace、span、日志或 dashboard”。这两类证据不能混为一谈：hook 是执行边界的拦截点，trace 是拦截或执行后留下的记录。

在本地 Codex / OMX 语境里，hook 还承担“把普通 CLI 会话变成 agent harness”的作用：它可以在用户提交 prompt、工具调用、停止和压缩时写入状态文件、更新 HUD、记录 turn log，或阻止不安全的结束。这个解释来自本地工程现象，不等于所有 Agent SDK 都必须用同样事件名。
