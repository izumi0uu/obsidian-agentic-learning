...
这种 runtime 的核心价值是把 Agent 行为从“隐含在 prompt 里的流程”变成“可检查、可恢复、可路由的图”。当节点失败、人工审批插入、工具超时或任务需要长期暂停时，state graph 可以保存状态并从确定边界继续。它的代价是建模成本和框架绑定：简单脚本不一定需要图；复杂生产流程则常需要图来承载可观测性、测试和恢复。
## 它解决什么问题
...
- Evidence type: official LangGraph/ADK docs + framework comparison map + engineering synthesis.
- Boundary: State Graph Runtime 是状态化流程执行层，不等于普通 prompt chain、provider SDK 或全部 control plane。
## 复习触发
