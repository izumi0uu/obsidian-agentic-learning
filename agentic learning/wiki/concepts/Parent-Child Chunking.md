...
Parent-Child Chunking 解决的是检索粒度和阅读粒度不一致的问题。小 chunk 更容易被 embedding 或关键词命中，但单独给模型可能缺上下文；大 chunk 可读性更好，却可能检索不准。父子切分通常让小 child chunk 参与索引和召回，命中后返回对应 parent 段落或文档片段给模型阅读。这样能在 recall、precision 和上下文完整性之间折中，但也需要处理 parent 过大、重复返回和来源引用粒度的问题。
## 它解决什么问题
...
- Evidence type: RAG chunking/retrieval interview notes + engineering synthesis.
- Boundary: Parent-Child Chunking 是索引/返回粒度设计，不等于普通 chunking、metadata filter、reranking 或 long-context 直接塞全文。
## 复习触发
