...
一个 benchmark 至少包含四层：任务集、运行环境、评分规则和报告口径。任务集决定测什么；运行环境决定模型或 Agent 能用哪些工具；评分规则决定怎么算成功；报告口径决定结果是否能和别人比较。[[Agent Evaluation Benchmark]] 是其中面向智能体行动能力的一支：[[GAIA Benchmark]] 更偏通用 assistant 复杂任务，[[SWE-bench]] 更偏真实 GitHub issue 的代码修复任务，[[BFCL]] 更偏 tool/function calling 能力，三者都提醒我们：Agent 能力需要放进具体任务协议里看。
...
BFCL 用函数文档、用户请求和 checker 评估 tool/function calling 能力。
...
2. [[Agent Evaluation Benchmark]]：面向 Agent 工具使用、环境交互、多步任务或协作能力的 benchmark 子类。
3. [[Eval Harness]]：运行 benchmark、收集 trace、执行 checker 和保存结果的工程系统。
4. [[Observability]] / [[Trace]]：帮助解释为什么某些任务失败，但不直接等于 benchmark 分数。
...
- Anchor: [[GAIA Benchmark#为什么收]] / [[SWE-bench#为什么收]] / [[BFCL - Berkeley Function Calling Leaderboard#关键事实]]
