...
LLM 主题先回答“模型一次调用如何读输入、生成输出、受什么边界限制”，再进入 Transformer 架构、推理缓存、训练/后训练、prompt-time reasoning 和 Agent 工程承载。最小入口是 [[LLM]]、[[Token]]、[[Context Window]]、[[KV Cache]]、[[Prompt]]、[[Prompt Engineering]]、[[Hallucination]] 和 [[LLM 输入输出基础边界对比]]。
...
1. **输入输出边界**：读 [[Token]]、[[Context Window]]、[[Prompt]]、[[Prompt Engineering]]、[[Hallucination]]，先知道一次模型调用能看到什么、怎样被指令、哪里会不可靠，以及 prompt 如何被工程化维护。
