...
LLM 主题先回答“自然语言任务是什么、模型一次调用如何读输入、生成输出、受什么边界限制”，再进入 Transformer 架构、推理缓存、训练/后训练、prompt-time reasoning 和 Agent 工程承载。最小入口是 [[NLP]]、[[LLM]]、[[Token]]、[[Context Window]]、[[KV Cache]]、[[Prompt]]、[[Prompt Engineering]]、[[Hallucination]] 和 [[LLM 输入输出基础边界对比]]。
