...
最小边界：[[Context Window]] 解决“一次调用能看见多少”；[[Context Rot]] 提醒“看得见不等于用得稳”；[[Memory]]、[[RAG]] 和 [[Context Engineering]] 解决“哪些信息应该被保存、检索、选择、排序并被正确使用”。
...
| 有效使用限制 | 噪声、冲突、无关材料、位置偏置、[[Context Rot]], Lost in the Middle | 正确证据在窗口里，但模型忽略、误读或被低质量片段干扰 | 需要排序、去重、重排、结构化提示、引用规则和 evidence selection |
...
[[GSSC Pipeline]] 可以作为这一步的最小工程流程：Gather 先收集候选上下文，Select 决定哪些真正进窗口，Structure 把它们组织成可读区块，Compress 只在超预算时做兜底。
...
系统工程侧突破包括外部记忆、RAG、任务分解、阶段摘要、[[Context Projection|state projection]]、trace、citation、权限过滤和上下文预算策略。[[130 01_AI 04_上下文工程与记忆 Wide Research Beyond the Context Window]] 的核心价值就在这里：突破窗口不只是增加 token，而是把单个巨大上下文改造成分层、分阶段、可检索、可审计的外部系统。

### 长时程任务侧

长时程任务还需要专门的 [[Long-Horizon Context Engineering]]：用 compaction 做上下文接力，用 structured note-taking 把关键状态和决策写到窗口外，用 sub-agent architectures 隔离搜索噪声并回传凝练摘要。它处理的是“跨很多轮行动仍保持目标、状态和证据一致”的问题，不等同于单次窗口扩容。
...
| 正确资料在窗口里但答错 | [[Context Rot]], [[Context Engineering]], [[RAG Evaluation]] | 重排、去重、证据分段、引用规则、降低噪声 |
| 候选信息很多但不知道怎么装配 | [[GSSC Pipeline]], [[Context Engineering]] | 先收集候选，再筛选、结构化，最后才压缩 |
...
- 概念卡：[[Context Window#概念详解]], [[Context Rot#概念详解]], [[Context Engineering#概念详解]], [[LLM 输入输出基础边界对比#Context Window vs Memory]], [[LLM 基础结构对比#Positional Encoding vs Context Window]]。
