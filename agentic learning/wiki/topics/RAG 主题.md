...
evidence:
  ...
  - "[[Semantic Search#证据锚点]]"
  - "[[Vector Similarity Metrics#证据锚点]]"
  - "[[Vector Search Algorithm#证据锚点]]"
  - "[[Approximate Nearest Neighbor Search#证据锚点]]"
  - "[[HNSW#证据锚点]]"
  - "[[Embedding Optimization#证据锚点]]"
  - "[[Matryoshka Embeddings#证据锚点]]"
  - "[[Embedding Quantization#证据锚点]]"
  - "[[Embedding Evaluation Benchmark#证据锚点]]"
  - "[[MTEB#证据锚点]]"
  - "[[Bi-Encoder#证据锚点]]"
related:
  - "[[RAG]]"
...
最小边界：[[RAG]] 是检索增强生成的架构模式；[[Retriever]]、[[Embedding]]、[[Semantic Search]]、[[Dense Retrieval]]、[[Bi-Encoder]]、[[Vector Similarity Metrics]]、[[Vector Search Algorithm]]、[[Approximate Nearest Neighbor Search]]、[[HNSW]]、[[TF-IDF]]、[[Sparse Retrieval]]、[[BM25]]、[[Vector Database]]、[[Top-K]]、[[Query Rewrite]]、[[Multi-Query Retrieval]]、[[HyDE]]、[[Step-back Prompting]]、[[Multi-Route Retrieval]]、[[Reciprocal Rank Fusion]]、[[Hybrid Search]]、[[Reranking]]、[[Cross-Encoder]] 是 retrieval 组件 / 参数 / query-side 策略；[[Embedding Optimization]]、[[Matryoshka Embeddings]]、[[Embedding Quantization]]、[[Embedding Evaluation Benchmark]] 和 [[MTEB]] 属于 embedding 选型、成本和评测边界；[[TTL]] 是资料、索引或缓存的 freshness 生命周期边界；[[Agent Harness 缓存分层与命中率]] 用来区分 embedding cache、retrieval cache、context cache 和 provider / serving cache 的控制层；[[GraphRAG]]、[[Agentic RAG]]、[[Corrective RAG]]、[[Self-RAG]] 是在不同位置改造 RAG 链路的类型；[[RAG Evaluation]] 负责检查检索排序指标（Hit@K / Recall@K / MRR / nDCG）、上下文、引用和答案质量。
...
- [[Agent Harness 缓存分层与命中率]]：把 RAG 里的 embedding / retrieval / context cache 命中率和 serving 层 KV / prompt cache 区分开，避免把成本优化误读为质量提升。
- [[RAGFlow]]：把端到端 RAG / context layer 平台作为工程样本，理解复杂文档解析、chunking、多路召回、重排、引用和可视化调试。
...
2. **再学 retrieval pipeline**：读 [[Document Ingestion]]、[[Chunking]]、[[Parent-Child Chunking]]、[[Embedding]]、[[Semantic Search]]、[[Dense Retrieval]]、[[Bi-Encoder]]、[[Vector Similarity Metrics]]、[[Vector Search Algorithm]]、[[Approximate Nearest Neighbor Search]]、[[HNSW]]、[[Embedding Optimization]]、[[Matryoshka Embeddings]]、[[Embedding Quantization]]、[[Embedding Evaluation Benchmark]]、[[MTEB]]、[[TF-IDF]]、[[Sparse Retrieval]]、[[BM25]]、[[Vector Database]]、[[Retriever]]、[[Top-K]]、[[Query Rewrite]]、[[Multi-Query Retrieval]]、[[HyDE]]、[[Step-back Prompting]]、[[Multi-Route Retrieval]]、[[Reciprocal Rank Fusion]]、[[Hybrid Search]]、[[Reranking]]、[[Cross-Encoder]]，训练自己按链路定位失败，而不是笼统说“RAG 不准”。
...
| 层 | 关键概念 | 主要问题 | 常见失败 |
|---|---|---|---|
| 资料入口 | [[Document Ingestion]], [[Chunking]], [[TTL]] | 原始资料怎样被解析、切分、去重、标注 metadata / 权限 / freshness | 表格丢结构、chunk 断开条件、权限标签丢失、过期资料继续被召回 |
| 表示与索引 | [[Embedding]], [[Semantic Search]], [[Dense Retrieval]], [[Bi-Encoder]], [[Vector Similarity Metrics]], [[Vector Search Algorithm]], [[Approximate Nearest Neighbor Search]], [[HNSW]], [[Embedding Optimization]], [[Matryoshka Embeddings]], [[Embedding Quantization]], [[Embedding Evaluation Benchmark]], [[MTEB]], [[TF-IDF]], [[Sparse Retrieval]], [[BM25]], [[Vector Database]], [[Agent Harness 缓存分层与命中率]] | 内容怎样变成可检索表示、怎样定义相似、怎样快速搜索、怎样控制向量成本并初筛模型 | 语义相似但事实不支持、query/chunk 分开编码导致细粒度判断不足、metric / normalization 配置不一致、ANN 召回损失、压缩导致漏召回、benchmark 误当业务评测、精确词项漏召回、分词/倒排索引问题、索引过期、metadata filter 不可用、embedding cache 版本漂移 |
| 召回与融合 | [[Retriever]], [[Top-K]], [[Query Rewrite]], [[Multi-Query Retrieval]], [[HyDE]], [[Step-back Prompting]], [[Query Planning]], [[Multi-Route Retrieval]], [[Reciprocal Rank Fusion]], [[Hybrid Search]], [[Parallel Search and Explicit Merging 检索模式]], [[Agent Harness 缓存分层与命中率]] | query 如何找到候选证据，是否需要改写、分解、规划、假设文档、缓存或并行多视角检索 | [[Top-K]] 漏掉关键文档、专有名词/编号匹配失败、query rewrite 偏题、HyDE 假设答案带偏、step-back 抽象过头、query plan 过度复杂、多 query 引入重复和噪声、retrieval cache 在语料或权限变化后继续返回旧候选 |
...
| [[Semantic Search]] | 我是在做语义搜索任务，还是完整 RAG？ | 语义相似能弥补 vocabulary mismatch，但不自动处理权限、引用和事实忠实 |
| [[Vector Similarity Metrics]] | query 和 chunk 的向量相似度到底怎么算？ | cosine、dot product、Euclidean / L2 distance 的排序含义和 normalization 相关 |
| [[Vector Search Algorithm]] | 有了相似度以后，系统怎样搜索向量？ | exact search、ANN、图索引、量化 / 聚类索引等算法层和向量数据库产品层的区别 |
| [[Approximate Nearest Neighbor Search]] | 向量很多时怎样避免全量扫描？ | ANN 用少量召回损失换速度；它解决 latency，不保证答案质量 |
| [[HNSW]] | 向量索引为什么能从粗到细找近邻？ | HNSW 是 ANN 图索引，不是向量数据库或 RAG 本身 |
| [[Embedding Optimization]] | embedding 成本应该在哪些层面优化？ | 维度、精度、索引、召回和 rescore 的质量 / 成本折中 |
| [[Matryoshka Embeddings]] | 是否需要通过截短维度降低向量成本？ | 只有明确支持可截短的 embedding 才适合；query / document 维度必须一致 |
| [[Embedding Quantization]] | 是否需要压缩向量存储和第一阶段检索成本？ | 量化会改变相似度排序，通常需要 full-precision rescore 或 rerank 兜底 |
| [[Embedding Evaluation Benchmark]] | embedding 模型初筛有哪些公共任务协议？ | benchmark 是模型初筛入口，不是业务 RAG 质量保证 |
| [[MTEB]] | embedding 模型初筛应该看什么公共基准？ | MTEB 是模型 benchmark，不是业务 RAG eval；必须用自己的 query / corpus 回归 |
...
| [[Bi-Encoder]] | 为什么向量检索能预计算文档向量？ | query/chunk 分开编码，快但缺少 token 级交互；和 Cross-Encoder 对比理解召回 / 精排 |
...
| [[HyDE]] | 问题和文档文体是否差太远？ | 生成 hypothetical document 作为 embedding 检索代理；不要把假设文档当事实 |
| [[Step-back Prompting]] | 具体问题是否需要背景原理？ | 先生成更抽象的背景问题，再回到原 query 的实体和限制 |
...
| [[RAGFlow]] | 我是否需要端到端 RAG 平台而不是只写代码组件？ | 复杂文档解析、可视化 chunking、多路召回、融合重排、引用、API/UI 和平台部署；不是 RAG 本身，也不是向量数据库 |
...
- RAG 不是事实正确性保险：资料可能错、检索可能漏、排序可能错、模型可能误读、引用可能不支持结论；[[TTL]] 只能提示 freshness 边界，不能替代事实校验。
- RAG 缓存命中率不是 RAG 质量指标：embedding cache、retrieval cache 和 context cache 能降低成本和延迟，但正确性仍要看召回样本、rerank、引用支持、权限过滤和 [[RAG Evaluation]]。
...
- [x] [[Semantic Search]]
- [x] [[Dense Retrieval]]
- [x] [[Vector Similarity Metrics]]
- [x] [[Vector Search Algorithm]]
- [x] [[Approximate Nearest Neighbor Search]]
- [x] [[HNSW]]
- [x] [[Embedding Optimization]]
- [x] [[Matryoshka Embeddings]]
- [x] [[Embedding Quantization]]
- [x] [[Embedding Evaluation Benchmark]]
- [x] [[MTEB]]
- [x] [[TF-IDF]]
- [x] [[Sparse Retrieval]]
- [x] [[BM25]]
...
3. [[Vector Similarity Metrics]]、[[Approximate Nearest Neighbor Search]] 和 [[HNSW]] 分别解决向量检索里的哪个问题？
4. [[Hybrid Search]] 和 [[Reranking]] 分别解决召回和排序里的什么问题？
5. [[MTEB]] 为什么不能替代自己的 RAG retrieval evaluation？
6. [[Multi-Route Retrieval]] 为什么不等于简单调大 Top-K 或只做 Hybrid Search？
7. 什么时候 [[Knowledge Graph]] / [[GraphRAG]] 值得引入，什么时候只是增加复杂度？
8. [[RAG]]、[[Memory]]、[[Context Engineering]] 三者都给模型提供信息，它们的最小边界是什么？
9. 如果检索内容里含有 prompt injection，RAG pipeline 应该在哪些点防护？
