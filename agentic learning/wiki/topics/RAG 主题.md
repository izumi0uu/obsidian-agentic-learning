...
最小边界：[[RAG]] 是检索增强生成的架构模式；[[Retriever]]、[[Embedding]]、[[Semantic Search]]、[[Dense Retrieval]]、[[Vector Similarity Metrics]]、[[Vector Search Algorithm]]、[[Approximate Nearest Neighbor Search]]、[[HNSW]]、[[TF-IDF]]、[[Sparse Retrieval]]、[[BM25]]、[[Vector Database]]、[[Top-K]]、[[Query Rewrite]]、[[Multi-Query Retrieval]]、[[HyDE]]、[[Step-back Prompting]]、[[Multi-Route Retrieval]]、[[Reciprocal Rank Fusion]]、[[Hybrid Search]]、[[Reranking]]、[[Cross-Encoder]] 是 retrieval 组件 / 参数 / query-side 策略；[[Embedding Optimization]]、[[Matryoshka Embeddings]]、[[Embedding Quantization]]、[[Embedding Evaluation Benchmark]] 和 [[MTEB]] 属于 embedding 选型、成本和评测边界；[[TTL]] 是资料、索引或缓存的 freshness 生命周期边界；[[GraphRAG]]、[[Agentic RAG]]、[[Corrective RAG]]、[[Self-RAG]] 是在不同位置改造 RAG 链路的类型；[[RAG Evaluation]] 负责检查检索排序指标（Hit@K / Recall@K / MRR / nDCG）、上下文、引用和答案质量。
...
2. **再学 retrieval pipeline**：读 [[Document Ingestion]]、[[Chunking]]、[[Parent-Child Chunking]]、[[Embedding]]、[[Semantic Search]]、[[Dense Retrieval]]、[[Vector Similarity Metrics]]、[[Vector Search Algorithm]]、[[Approximate Nearest Neighbor Search]]、[[HNSW]]、[[Embedding Optimization]]、[[Matryoshka Embeddings]]、[[Embedding Quantization]]、[[Embedding Evaluation Benchmark]]、[[MTEB]]、[[TF-IDF]]、[[Sparse Retrieval]]、[[BM25]]、[[Vector Database]]、[[Retriever]]、[[Top-K]]、[[Query Rewrite]]、[[Multi-Query Retrieval]]、[[HyDE]]、[[Step-back Prompting]]、[[Multi-Route Retrieval]]、[[Reciprocal Rank Fusion]]、[[Hybrid Search]]、[[Reranking]]、[[Cross-Encoder]]，训练自己按链路定位失败，而不是笼统说“RAG 不准”。
...
| 表示与索引 | [[Embedding]], [[Semantic Search]], [[Dense Retrieval]], [[Vector Similarity Metrics]], [[Vector Search Algorithm]], [[Approximate Nearest Neighbor Search]], [[HNSW]], [[Embedding Optimization]], [[Matryoshka Embeddings]], [[Embedding Quantization]], [[Embedding Evaluation Benchmark]], [[MTEB]], [[TF-IDF]], [[Sparse Retrieval]], [[BM25]], [[Vector Database]] | 内容怎样变成可检索表示、怎样定义相似、怎样快速搜索、怎样控制向量成本并初筛模型 | 语义相似但事实不支持、metric / normalization 配置不一致、ANN 召回损失、压缩导致漏召回、benchmark 误当业务评测、精确词项漏召回、分词/倒排索引问题、索引过期、metadata filter 不可用 |
| 召回与融合 | [[Retriever]], [[Top-K]], [[Query Rewrite]], [[Multi-Query Retrieval]], [[HyDE]], [[Step-back Prompting]], [[Query Planning]], [[Multi-Route Retrieval]], [[Reciprocal Rank Fusion]], [[Hybrid Search]], [[Parallel Search and Explicit Merging 检索模式]] | query 如何找到候选证据，是否需要改写、分解、规划、假设文档或并行多视角检索 | [[Top-K]] 漏掉关键文档、专有名词/编号匹配失败、query rewrite 偏题、HyDE 假设答案带偏、step-back 抽象过头、query plan 过度复杂、多 query 引入重复和噪声 |
