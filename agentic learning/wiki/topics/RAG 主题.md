...
最小边界：[[RAG]] 是检索增强生成的架构模式；[[Retriever]]、[[Embedding]]、[[Vector Database]]、[[Hybrid Search]]、[[Reranking]] 是 retrieval 组件；[[GraphRAG]]、[[Agentic RAG]]、[[Corrective RAG]]、[[Self-RAG]] 是在不同位置改造 RAG 链路的类型；[[RAG Evaluation]] 负责检查检索、上下文、引用和答案质量。
...
2. **再学 retrieval pipeline**：读 [[Document Ingestion]]、[[Chunking]]、[[Embedding]]、[[Vector Database]]、[[Retriever]]、[[Hybrid Search]]、[[Reranking]]，训练自己按链路定位失败，而不是笼统说“RAG 不准”。
3. **再学 RAG 类型边界**：读 [[RAG 类型对比]]，把 [[GraphRAG]]、[[Agentic RAG]]、[[Agentic Retrieval]]、[[Corrective RAG]]、[[Self-RAG]] 放到“改造哪一段链路”的坐标里。
4. **最后学治理和评估**：读 [[RAG Evaluation]]、[[Context Engineering]]、[[Indirect Prompt Injection]]，理解 citation、权限、上下文污染、prompt injection 和回归测试为什么是生产 RAG 的关键。
...
| 表示与索引 | [[Embedding]], [[Vector Database]] | 内容怎样变成可检索表示并被快速搜索 | 语义相似但事实不支持、索引过期、metadata filter 不可用 |
| 召回与融合 | [[Retriever]], [[Hybrid Search]] | query 如何找到候选证据 | top-k 漏掉关键文档、专有名词/编号匹配失败、query rewrite 偏题 |
| 排序与上下文 | [[Reranking]], [[Context Engineering]] | 哪些证据进入上下文、按什么顺序、带哪些引用 | 正确证据排太后、上下文太噪、引用和 chunk 对不上 |
| 生成与校验 | [[RAG]], [[RAG Evaluation]] | 模型如何基于证据回答，答案是否被引用支持 | 有证据但模型误读、答案编造、citation 不支持结论 |
| 高级改造 | [[GraphRAG]], [[Agentic RAG]], [[Corrective RAG]], [[Self-RAG]] | 是否需要图结构、检索决策、证据质量门或自适应检索 | 复杂度、延迟、trace/eval 成本上升，错误更难定位 |
...
| [[Vector Database]] | 我需要什么样的向量索引和过滤能力？ | 存储/搜索向量，不等于完整 RAG；要看 metadata、删除、更新、权限 |
| [[Retriever]] | 系统怎样找到候选证据？ | query rewrite、filter、top-k、召回、去重、多源检索 |
| [[Hybrid Search]] | 纯向量是否漏掉精确匹配？ | 向量 + 关键词/全文互补，尤其适合实体名、错误码、产品编号 |
| [[Reranking]] | 找到的候选谁更该进上下文？ | rerank 只能重排已召回候选，不能凭空找回漏掉的证据 |
| [[RAG Evaluation]] | 怎么知道 RAG 错在哪里？ | retrieval、context、generation、citation 分层评估 |
...
| [[Knowledge Graph]] + [[Neo4j]] | 图数据库 / 知识图谱实现层 | 需要显式实体、关系、Cypher / 图遍历、向量 + 全文 + 图组合 | Neo4j 是实现生态，不是 GraphRAG 定义本身 |
| [[Agentic Retrieval]] | 检索层 query planning | 复杂企业搜索、多源检索、子问题分解 | 更偏 retrieval 层，不等于完整 Agentic RAG |
...
| 专有名词、函数名、错误码找不到 | [[Hybrid Search]]、metadata filter、关键词/全文索引 | 不要只调 embedding 相似度 |
...
- [ ] RAG citation faithfulness：答案引用是否真的支持结论，是否应独立成卡？
- [ ] RAG access control：权限过滤、租户隔离和检索前过滤是否应该独立成安全概念卡？
- [ ] Query rewrite / query planning：作为 [[Retriever]] / [[Agentic Retrieval]] 的子概念，是否需要单独沉淀？
- [ ] Graph construction evaluation：[[Knowledge Graph]] / [[GraphRAG]] 的抽取质量如何评估？
...
- Concept anchors: [[RAG#证据锚点]], [[Retriever#证据锚点]], [[Document Ingestion#证据锚点]], [[Chunking#证据锚点]], [[Embedding#证据锚点]], [[Vector Database#证据锚点]], [[Hybrid Search#证据锚点]], [[Reranking#证据锚点]], [[RAG Evaluation#证据锚点]], [[Knowledge Graph#证据锚点]], [[GraphRAG#证据锚点]], [[Neo4j#证据锚点]]。
...
4. 什么时候 [[Knowledge Graph]] / [[GraphRAG]] 值得引入，什么时候只是增加复杂度？
5. [[RAG]]、[[Memory]]、[[Context Engineering]] 三者都给模型提供信息，它们的最小边界是什么？
6. 如果检索内容里含有 prompt injection，RAG pipeline 应该在哪些点防护？
