...
这页主轴仍是 [[Plan-and-Solve Prompting]]、[[ReAct]]、[[Reflexion]] 三张地基卡：它们都在改善“模型怎样把复杂任务做得更可靠”，但介入点不同。[[ReWOO]] 作为相邻补充，用来切开“先计划证据槽，但不让每轮 Observation 回流”的中间地带。
...
相邻补充：[[ReWOO]] 可以插在 Plan-and-Solve 和 ReAct 之间看。它先规划 `Plan / #E` evidence slots，再让 Worker 调工具填 evidence，最后由 Solver 汇总；它保留工具取证，但不让每轮 Observation 驱动下一步 reasoning。

## 为什么这组值得对比

- 混淆风险高：这些方法都会出现“推理、计划、行动、观察、反思”这些词，容易被统称为“Agent 会自己思考”。
- 共同问题域相近：都试图减少复杂任务中的盲目生成、漏步骤、外部事实缺失或重复失败。
- 介入点不同：一个在求解前，一个在执行中，一个在执行后。
- 证据足够：三张主轴概念卡、ReWOO 补充卡和对应 paper source note 都已有定义、机制和边界锚点。
- 现代工程价值明显：它们分别对应现代系统里的 plan artifact、evidence-slot planning、tool/action loop、evaluation + memory / retry policy。

边界：这页不是把这些方法强行归成一个家族；它们只是都帮助理解“复杂任务可靠性”的不同切面。

## 共同问题域

共同问题可以概括为：LLM 直接生成答案时，可能漏步骤、缺外部反馈、失败后重复犯错。这些方法都在给“直接生成”加结构，但加的位置不同。
...
这也是它们最适合一起学习的原因：它们像把同一条任务时间线切成几个位置——行动前计划、证据槽规划与取证、行动中观察校正、行动后反思。
...
| [[ReWOO]] | 求解前规划 evidence slots，执行中批量/分步取证，最后合成 | `Goal -> Planner(Plan/#E) -> Worker(Evidence) -> Solver(Answer)` | 任务目标、工具语境、预设 evidence slots | 计划蓝图、工具 evidence、最终答案 | [[ReWOO - Decoupling Reasoning from Observations for Efficient Augmented Language Models#需要我读的内容]] |
...
### Plan-and-Solve vs ReWOO

[[Plan-and-Solve Prompting]] 的 plan 通常不调用外部工具；它的 plan 和 solve 都发生在同一次回答内部。[[ReWOO]] 则会在 plan 里预留 `#E` evidence slots，让 Worker 调工具填证据，再让 Solver 合成答案。它比 Plan-and-Solve 更接近 tool-augmented reasoning，但还没有进入 ReAct 式“每次 observation 改变下一步 thought”的循环。

### ReAct vs ReWOO

[[ReAct]] 把 Observation 当成当前 trajectory 的下一步决策输入；[[ReWOO]] 把 Observation 限制在 evidence filling 和 final solve。前者更适合工具结果会改变路线的任务，后者更适合证据需求能预先规划、主要成本来自重复 prompt 的任务。
...
ReWOO
Goal/Context -> Planner(Plan/#E) -> Worker(Tool Evidence) -> Solver -> Answer
...
Before/evidence collection: ReWOO style evidence-slot planning and tool evidence filling
...
- [[ReWOO]] 卡和 source note 支持 Planner / Worker / Solver 分层、`Plan / #E` evidence slots 和 prompt redundancy reduction；DORA 反例提醒它不适合强中间反馈任务。
...
| 多跳取证，证据需求可预判，主要瓶颈是重复 prompt 或串行工具成本 | [[ReWOO]] | 先规划 evidence slots，再让 Worker 收证据，最后 Solver 合成 | 如果工具结果会改变后续路线，移除 observation feedback 会掉可靠性 |
...
- Definition / mechanism anchors:
  ...
  - [[Reflexion - Language Agents with Verbal Reinforcement Learning#必读块 1：Abstract / linguistic feedback 而非权重更新]]
  - [[Reflexion - Language Agents with Verbal Reinforcement Learning#必读块 2：Abstract / episodic memory buffer]]
- Concept synthesis anchors: [[ReAct#概念详解]], [[Plan-and-Solve Prompting#概念详解]], [[ReWOO#概念详解]], [[Reflexion#概念详解]]
...
2. ReWOO 为什么比 Plan-and-Solve 多工具证据，却仍然不是 ReAct？
3. ReAct 中的 Observation 和 Reflexion 中的 evaluator feedback 最小区别是什么？
4. 为什么 Reflexion 的 reflective text 不等于普通 [[Reasoning Trace]]？
5. 给一个浏览器 Agent 失败案例：哪些部分适合用 plan，哪些部分适合用 ReAct loop，哪些部分适合写成 Reflexion experience？
6. 如果 evaluator 给错反馈，这组方法里哪个机制最容易把错误固化到下一轮？为什么？
